...
export function createIssue(db: Database.Database, input: z.infer<typeof CreateIssueInput>): IssueRow {
  ...
  db.prepare(
    `INSERT INTO issues (id, task_id, kind, severity, body, resolution, created_at, resolved_at)
     VALUES (?, ?, ?, ?, ?, NULL, ?, NULL)`,
  ).run(id, parsed.task_id ?? null, parsed.kind, parsed.severity, parsed.body, now);
...
export function resolveIssue(db: Database.Database, input: z.infer<typeof ResolveIssueInput>): IssueRow {
  const parsed = ResolveIssueInput.parse(input);

  const existing = db.prepare('SELECT * FROM issues WHERE id = ?').get(parsed.id) as IssueRow | undefined;
  ...
  const now = Date.now();
  db.prepare('UPDATE issues SET resolution = ?, resolved_at = ? WHERE id = ?').run(parsed.resolution, now, parsed.id);
  ...
  return db.prepare('SELECT * FROM issues WHERE id = ?').get(parsed.id) as IssueRow;
}
