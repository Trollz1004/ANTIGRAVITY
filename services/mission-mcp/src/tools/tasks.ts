...
export const ListTasksInput = z.object({
  status: z.enum(['pending', 'in_progress', 'done', 'failed', 'blocked']).optional(),
  parent_task_id: z.string().optional(),
  assigned_agent_id: z.string().optional(),
...
export function createTask(db: Database.Database, input: z.infer<typeof CreateTaskInput>): TaskRow {
...
export function listTasks(db: Database.Database, input: z.infer<typeof ListTasksInput>): TaskRow[] {
  ...
  if (parsed.tag !== undefined) {
    conditions.push('(description LIKE ? OR title LIKE ?)');
...
export function updateTask(db: Database.Database, input: z.infer<typeof UpdateTaskInput>): TaskRow {
  const parsed = UpdateTaskInput.parse(input);

  const existing = db.prepare('SELECT * FROM tasks WHERE id = ?').get(parsed.id) as TaskRow | undefined;
  ...
  return db.prepare('SELECT * FROM tasks WHERE id = ?').get(parsed.id) as TaskRow;
}
