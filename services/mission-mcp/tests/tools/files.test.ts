...
import { resolveSafePath, readFileTool, writeFileTool, patchFileTool } from '../../src/tools/files.js';
...
describe('files', () => {
  ...
  it('resolveSafePath: rejects absolute Unix paths', () => {
    expect(() => resolveSafePath('/etc/passwd', tmpRoot)).toThrow('Absolute paths are not allowed');
  });

  it('resolveSafePath: rejects absolute Windows paths', () => {
    expect(() => resolveSafePath('C:\\Windows\\system32', tmpRoot)).toThrow('Absolute paths are not allowed');
  });

  it('resolveSafePath: rejects path traversal', () => {
    expect(() => resolveSafePath('../../etc/passwd', tmpRoot)).toThrow('Path traversal rejected');
  });
  ...
  it('readFileTool: throws for missing file', () => {
    ...
    expect(() => readFileTool(db, { path: 'nonexistent.txt' })).toThrow('File not found');
  });
  ...
  it('writeFileTool: creates new file', () => {
    ...
    expect(readFileSync(join(tmpRoot, 'new-file.txt'), 'utf-8')).toBe('Fresh content');
  });
  ...
  it('writeFileTool: overwrites existing file', () => {
    ...
    expect(readFileSync(join(tmpRoot, 'existing.txt'), 'utf-8')).toBe('New content');
  });
  ...
  it('patchFileTool: throws on non-applying diff', () => {
    ...
    const diff =
      ['--- a/unmatch.txt', '+++ b/unmatch.txt', '@@ -1,1 +1,1 @@', '-Hello world', '+Hello ANTIGRAVITY'].join('\n') +
      '\n';

    expect(() => patchFileTool(db, { path: 'unmatch.txt', diff })).toThrow('Patch did not apply cleanly');
  });
});
