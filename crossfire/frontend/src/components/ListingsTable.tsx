...
export default function ListingsTable() {
  ...
  const [form, setForm] = useState({ title: '', ebay_price: '', cost_basis: '', condition: 'Used', quantity: '1', sku: '' })

  const load = async () => {
    try { setListings(await getListings() as Listing[]) } catch {}
  }
    ...
    <div>
      ...
      {showAdd && (
        <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-6 grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <input placeholder="eBay Price" type="number" value={form.ebay_price} onChange={e => setForm({ ...form, ebay_price: e.target.value })}
            className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white outline-none focus:border-orange-500" />
          <input placeholder="Cost Basis" type="number" value={form.cost_basis} onChange={e => setForm({ ...form, cost_basis: e.target.value })}
            className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white outline-none focus:border-orange-500" />
              ...
              {listings.map(l => (
                <tr key={l.id} className="border-b border-white/5 hover:bg-white/5 transition">
                  ...
                  <td className="py-3 px-2 text-center">
                    <span className={`text-xs px-2 py-0.5 rounded ${STATUS_COLORS[l.status] || 'bg-white/10 text-white/40'}`}>
                      {l.status}
                    </span>
                  </td>
                  <td className="py-3 px-2 text-center">
