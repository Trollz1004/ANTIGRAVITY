...
export const calcPrices = (
  ebay_price: number,
  cost_basis: number = 0,
  shipping_cost: number = 6.50,
  materials_cost: number = 1.50,
) =>
...
export const createListing = (data: any) =>
  fetchJson('/listings/', { method: 'POST', body: JSON.stringify(data) });
export const deleteListing = (id: number) =>
  fetchJson(`/listings/${id}`, { method: 'DELETE' });
...
export const bestRate = (weight_oz: number) =>
  fetchJson(`/shipping/best-rate/${weight_oz}`);
...
// CSV Import/Export
export const importCsvFile = async (file: File, costBasis = 0, shippingCost = 6.50, materialsCost = 1.50) => {
...
export const importCsvText = (csvText: string, costBasis = 0, shippingCost = 6.50, materialsCost = 1.50) =>
  fetchJson('/csv/import-text', {
    method: 'POST',
    body: JSON.stringify({ csv_text: csvText, cost_basis: costBasis, shipping_cost: shippingCost, materials_cost: materialsCost }),
  });

export const getCurrentCsv = () => fetchJson('/csv/current');

export const downloadCrossfireCsv = () =>
  window.open(`${BASE}/csv/export/crossfire`, '_blank');
...
export const downloadSquareCsv = () =>
  window.open(`${BASE}/csv/export/square`, '_blank');
