...
const CHECKS = [
  ...
  { id:'omni',            name:'OmniRoute LB',       url:'http://127.0.0.1:20128/',              kind:'port', group:'Services' },
  ...
  { id:'tunnel_public',   name:'Cloudflare Tunnel Public', url:'https://paperclip.youandinotai.com/healthz', kind:'url', group:'Tunnels' },
  { id:'tunnel_local',    name:'Cloudflare Tunnel Process', check:'tunnel', kind:'process', group:'Tunnels' }
];
...
const FIXES = {
  ...
  ollama:         { label:'Ensure Ollama',          cmd:`powershell.exe -NoProfile -ExecutionPolicy Bypass -File "${REPO}\\scripts\\ollama-start.ps1"` },
  ...
  if (url.pathname === '/api/status') {
    ...
    for (const c of CHECKS) {
      ...
      if (c.kind === 'url') {
        ...
        try {
          const out = execSync(`powershell.exe -NoProfile -Command \"Get-CimInstance Win32_Process -Filter 'Name='cloudflared.exe'' -ErrorAction SilentlyContinue | Measure-Object | Select-Object -Expand Count\"`, { encoding:'utf8', timeout:4000 }).trim();
          ok = parseInt(out||'0',10) > 0;
        } catch { ok=false; }
      }
      results.push({ ...c, ok, status, ms, body });
    }
    res.writeHead(200, {'Content-Type':'application/json'}); res.end(JSON.stringify({ ts: Date.now(), results }));
    return;
  }
  if (url.pathname.startsWith('/api/fix/')) {
    const id = url.pathname.split('/')[3];
    const fix = FIXES[id];
    if (!fix || !fix.cmd) { res.writeHead(404); res.end(JSON.stringify({ ok:false, error:'no auto-fix' })); return; }
    try {
      const out = execSync(fix.cmd, { encoding:'utf8', timeout:120000, stdio:['ignore','pipe','pipe'] });
      res.writeHead(200, {'Content-Type':'application/json'}); res.end(JSON.stringify({ ok:true, cmd: fix.label, out: String(out).slice(0,4000) }));
    } catch (e) {
      res.writeHead(200, {'Content-Type':'application/json'}); res.end(JSON.stringify({ ok:false, cmd: fix.label, error: String(e.message||e).slice(0,2000) }));
    }
    return;
  }
