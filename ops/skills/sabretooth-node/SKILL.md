...
- **Date app is FROZEN and FOR SALE** (2026-09-16). Keep-alive only: API :8000, frontend :3200, tunnel, Postgres, Redis. Nothing else. Sale state lives in `ops/sale/YOUANDINOTAI-SALE-LISTING.md` (Status log) and `ops/sale/OUTREACH-TARGETS.md`. Never propose date-app feature work. **Marketing was unfrozen 2026-09-19**: posts go through the JARVIS approval inbox (four mechanical checks, Fable-voice drafts, a Sonnet rubric review), and Joshua delegated approval to this lane the same day ("fable approves is fine"), two per brand per day. Hermes drafts and files under `PROMPT-FOR-HERMES-MARKETING-2026-09-20.md` in the drop box; it never posts or approves. Reddit publishing waits on Joshua's one-time `drift reddit-setup`.
...
- **Alienware over SSH (since 2026-09-18):** `ssh -i ~/.ssh/id_ed25519_alienware joshi@192.168.0.40 '<powershell>'`, key-only, PowerShell is the remote shell, host ALIENWARE-11thgeni7, Windows 11 Home. Use it to read, never to do that node's protected work; the Alienware Claude owns its House, drift, runbook, and skill. Known layout there: `C:\DREAM\{dream-online,hermes,adapters}`, vault at `C:\DREAM\dream-online\DREAM-ONLINE`; Unreal is NOT at `C:\DREAM\UE_5.8`. Its :20128 is a separate, empty OmniRoute install with no providers; every lane still routes only to Sabretooth's http://192.168.0.8:20128/v1. God's Eye group "Alienware node (DREAM)" probes its 3000, 9119, 9127, 9133, 9150, 11434, 20128, 22.
