...
$PaperclipTunnelConfig = 'c:\users\joshl\.cloudflared\hermes-t5500.yml'
...
$BootstrapUrls = @(
  'http://localhost:3120',
  'https://paperclip-clean.youandinotai.com',
...
function Ensure-PaperclipPortAvailable {
  try {
    $healthResponse = Invoke-WebRequest -Uri 'http://127.0.0.1:3120/api/health' -TimeoutSec 2 -UseBasicParsing
...
function Start-PaperclipWithRetry {
  ...
  for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
    ...
    for ($i = 0; $i -lt 20; $i++) {
      try {
        $response = Invoke-WebRequest -Uri 'http://127.0.0.1:3120/api/health' -TimeoutSec 3 -UseBasicParsing
...
function Wait-ForPaperclip {
  Log 'Waiting for Paperclip to answer on http://127.0.0.1:3120/api/health...'
  for ($i = 0; $i -lt 30; $i++) {
    try {
      $response = Invoke-WebRequest -Uri 'http://127.0.0.1:3120/api/health' -TimeoutSec 3 -UseBasicParsing
      if ($response.StatusCode -ge 200 -and $response.StatusCode -lt 500) {
...
try {
  ...
  Log '  Paperclip:   http://localhost:3120'
  Log '  Public HQ:   https://paperclip-clean.youandinotai.com'
