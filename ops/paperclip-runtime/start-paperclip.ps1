...
$envPaths = @(
    ...
    'E:ANTIGRAVITYbriefings\MASTER-UNIVERSAL-ENV-TROLLZ1004.env'
)
$loaded = $false
...
if (-not $env:PAPERCLIP_PUBLIC_URL) {
    $env:PAPERCLIP_PUBLIC_URL = 'http://localhost:3120'
}
if (-not $env:BETTER_AUTH_TRUSTED_ORIGINS) {
    $env:BETTER_AUTH_TRUSTED_ORIGINS = 'http://localhost:3120,http://127.0.0.1:3120,https://paperclip-clean.youandinotai.com'
}
if (-not $env:PORT) {
    $env:PORT = '3120'
}
...
try {
    & 'C:\Users\joshl\AppData\Roaming\npm\paperclipai.cmd' run --config $PaperclipConfig --instance default --bind loopback *>> $LogFile 2>&1
} catch {
