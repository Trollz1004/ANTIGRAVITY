...
**Use:** Paste the block between the `=== PROMPT ===` markers into claude.ai Designer. **One shot.** Designer returns drop-in TypeScript files for the existing OpusPawClaw flagship at `D:\Antigravity\joshuaclaw-flagship-beta-testing\` — no new app, no rebuild.
...
You are extending an existing flagship desktop app called **OpusPawClaw** (codename `pawclaw-elite-v1`) at `D:\Antigravity\joshuaclaw-flagship-beta-testing\`. Vite + Electron + React 19 + Tailwind v4. Owner: Joshua Coleman, mission tag `#UntilNoKidInNeed`. ~14 days runway. Mission: medical care for children.
...
1. Drop the 5 files into `D:\Antigravity\joshuaclaw-flagship-beta-testing\src\` at the paths Designer specifies.
