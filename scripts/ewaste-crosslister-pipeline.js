...
function title80(parts) {
  ...
  t = t.replace(/\b(Intel|AMD|Processor|CPU)\b/gi, "").replace(/\s+/g, " ").trim();
  if (t.length <= 80) return t;
...
function main() {
  ...
  const listings = selected.map((r, idx) => {
    ...
    const storage = r.storage_capacity_gb ? `${r.storage_capacity_gb}GB ${r.storage_type || "Storage"}` : r.storage_type || "";
    ...
    const htmlDescription = [
      ...
      `<p><strong>Condition:</strong> Grade ${escHtml(grade)}. ${escHtml(r.condition_at_dropoff || g.grade_notes || "Used device with normal cosmetic wear.")}</p>`,
  ...
  fs.writeFileSync(jsonPath, JSON.stringify({ generated_at: now.toISOString(), batch_id: batchId, count: listings.length, listings }, null, 2) + "\n", "utf8");
