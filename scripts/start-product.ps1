# ═══════════════════════════════════════════════════════════════════════════
#  start-product-dashboard.ps1   ·   ANTIGRAVITY · Business-only product operations Product Transparency
...
try {
  ...
  Clear-Host
  Write-Banner 'ANTIGRAVITY · Business-only product operations · Product Transparency Launcher' 'Magenta'
  ...
  $productFile = Join-Path $ScriptDir 'Product Transparency.html'
  if (Test-Path $productFile) {
    Write-OK "Dashboard file found: Product Transparency.html"
  } else {
    Write-Warn "Dashboard file not at: $productFile"
    Write-Info "Script will still run health checks and the static server."
    Write-Info "Put 'Product Transparency.html' next to this script if you want it served."
  }
  ...
  $Url = "http://localhost:$ServePort/Product%20Transparency.html"
  ...
  switch ($ServerMode) {
    ...
    'none' {
      ...
      Write-Info "You can still open the file directly: $productFile"
    }
  }
  ...
  } elseif (Test-Path $productFile) {
    Invoke-Safe 'open file' {
      Start-Process $productFile
      Write-OK "Opened file directly: $productFile"
    } | Out-Null
  }
