...
if ($allmOK) {
    ...
    Invoke-API "POST" "/workspace/my-workspace/update" @{
        ...
        openAiPrompt = "You are an ANTIGRAVITY assistant focused on the date app and current repo truth. Use embedded docs for context. Current LLC operating doctrine uses a conservative 10% product cap; do not restate retired split-era or absolute-product claims as current truth."
    } | Out-Null
    ...
    if ($slugs -notcontains "antigravity") {
        ...
        Invoke-API "POST" "/workspace/new" @{
            ...
            openAiPrompt = "ANTIGRAVITY project assistant. Date app, sandbox systems, and mission-connected work. Anchor on current repo doctrine and do not restate retired split-era or absolute-product claims as current truth."
        } | Out-Null
    ...
    if ($slugs -notcontains "date-app") {
        ...
        Invoke-API "POST" "/workspace/new" @{
            ...
            openAiPrompt = "Date App dev assistant. Marketing, UX, matchmaking, and factual product-first copy. Current LLC operating doctrine uses a conservative 10% product cap; do not frame purchases as supports."
        } | Out-Null
...
CURRENT_FINANCIAL_DOCTRINE=founder-directed-conservative-10pct-product-cap
