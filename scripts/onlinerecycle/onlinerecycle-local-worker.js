...
function usage() {
  ...
  console.log(
    '  node scripts/onlinerecycle/onlinerecycle-local-worker.js reply-draft --kind general|dropoff|pickup --source FILE [--model MODEL]',
  );
}
  ...
  try {
    ...
    return listings
      ...
      .map((item) => {
        ...
        const price =
          item.suggested_price_usd == null || item.suggested_price_usd === ''
            ? 'price not set'
...
function replyTemplate(kind, sourcePath, fields) {
  ...
  const locationLine = kind === 'pickup' ? locationParts.join(', ') : null;
  ...
  const internalActions = [
    ...
    notesValue
      ? `- Keep the noted constraint in view: ${notesValue}`
      : '- Check for any access, timing, or handling constraints before confirming.',
  ].join('\n');
  ...
  const replyLines = [
    ...
    `Hello ${name},`,
    '',
    `Thanks for sending your OnlineRecycle ${
      kind === 'pickup' ? 'pickup' : kind === 'dropoff' ? 'drop-off' : ''
    } intake. I have your request and the details below:`,
    '',
...
async function marketingPack(model) {
  ...
  const listingLines = listings.length
    ? listings
        .map(
          (item) =>
            `- ${String(item.title || 'Untitled item').trim()} | $${item.suggested_price_usd || 'TBD'} | ${String(
              item.listing_format || 'buy_it_now',
            ).trim()}`,
        )
        .join('\n')
