...
const SKIP = new Set([
  ...
  '.venv', 'venv', '__pycache__', 'AppData', '.cache', 'dist', 'build',
]);
const MAX_DEPTH = 5;
...
function gitInfo(dir) {
  ...
  if (upstream) {
    ...
    if (c) { const [b, a] = c.split(/\s+/); behind = +b; ahead = +a; }
  }
  const lastCommit = sh('git', ['log', '-1', '--format=%cI|%s'], dir);
  return {
    remote, branch, head, upstream,
...
function walk(dir, depth) {
  ...
  const kinds = MARKERS.filter(([, test]) => { try { return test(dir); } catch { return false; } }).map(([k]) => k);
  if (kinds.length && !seen.has(dir.toLowerCase())) {
    seen.add(dir.toLowerCase());
    let mtime = null, sizeHint = 0;
    try { mtime = statSync(dir).mtime.toISOString(); } catch {}
    try { sizeHint = entries.length; } catch {}
