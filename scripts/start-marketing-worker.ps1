...
if (-not $env:NODE_NAME) { $env:NODE_NAME = '9020' }
if (-not $env:AGENT_HUB_URL) { $env:AGENT_HUB_URL = 'http://192.168.0.8:3130' }

Set-Location $serviceDir
"[$(Get-Date -Format o)] Starting marketing-worker port=$env:PORT node=$env:NODE_NAME" | Add-Content -Path $logPath -Encoding UTF8
