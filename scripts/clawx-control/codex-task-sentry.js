          ...
          spawn_on_done: [
            {
              ...
              prompt:
                ...
                'Tone must be transparent, respectful, local, and focused on fast intake plus responsible recycling without non-product public-benefit language.',
            },
            {
              id: `EW-RESPONSES-${day}`,
              ...
              prompt:
                ...
                'Keep them direct, respectful, local to Central Florida, and free of non-product public-benefit language. Mention Square booking only where useful.',
            },
          ],
        },
        {
...
function runCodexTask(task, maxResultChars) {
  ...
  const useFullAccess = task.dangerously_bypass === undefined ? fullAccessByDefault : Boolean(task.dangerously_bypass);
...
async function dispatchTask(task, forceExecutor, openclawUrl, ollamaUrl, maxResultChars) {
  const executor = String(forceExecutor || task.executor || 'codex')
    .toLowerCase()
    .trim();
  if (executor === 'codex') {
...
async function runOneCycle(options, logger) {
  ...
  if (!dispatch.ok && Array.isArray(options.fallbackChain) && options.fallbackChain.length > 0) {
    ...
    for (const fallbackExec of options.fallbackChain) {
      ...
      logger.warn(
        `Executor '${attempts[attempts.length - 1].executor}' failed for ${taskId}; trying '${fallbackExec}'.`,
      );
...
function printHelp() {
  ...
  console.log(
    '  node scripts/clawx-control/codex-task-sentry.js --run-once --fallback-chain ollama --no-codex-fallback',
  );
