...
SEEN_FILE = DATA_DIR / "stripe-seen-charges.json"
...
def _stripe_get(endpoint, params=None):
    ...
    try:
        ...
        if resp.status_code == 200:
            return resp.json()
        log.warning(f"Stripe API {resp.status_code}: {resp.text[:200]}")
    except Exception as e:
        log.error(f"Stripe request failed: {e}")
    return None


def check_new_payments():
    """Check Stripe for new successful charges. Returns list of new charges."""
    if not STRIPE_KEY:
        return []
    ...
    data = _stripe_get("charges", {"limit": 20, "status": "succeeded"})
    if not data:
        return []
...
def get_revenue_snapshot():
    """Get current Stripe balance and customer count."""
    if not STRIPE_KEY:
        return None

    balance = _stripe_get("balance")
    customers = _stripe_get("customers", {"limit": 1})
    subs = _stripe_get("subscriptions", {"status": "active", "limit": 1})
