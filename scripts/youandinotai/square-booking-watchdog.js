...
function checkOnce(state) {
  ...
  if (!newEvents.length) {
    console.log(
      `NO_NEW_BOOKINGS rows=${rows.length} csv_exists=${fs.existsSync(BOOKINGS_CSV) ? 'yes' : 'no'} time=${nowIso()}`,
    );
  ...
  for (const event of newEvents) {
    ...
    const line = `NEW_BOOKING event_id=${eventId} booking_id=${event.booking_id || ''} status=${
