...
6. `-impact-ledger-template.csv` (one row per sold eBay order/transaction)
7. `-impact-weekly-summary-template.csv` (one row per reporting week)

Minimum rule: do not list on eBay until testing passes and data wipe is verified.

## Post-Sale Ledger Rules

The row-level ledger maps each sold eBay item to  impact and payout evidence. Keep one row per sold order/transaction, even if multiple orders share the same listing.

Required mapping fields:

- `intake_id`, `asset_tag`, `ebay_item_id`, `ebay_order_id`, `ebay_transaction_id`
- `sold_date`, `ebay_payout_id`, `payout_date`
- `_recipient`, `contribution_batch_id`, `contribution_status`, `reconciliation_status`
...
- `_allocated_usd = round(net_proceeds_usd * (_share_pct / 100), 2)`
- `_variance_usd = _allocated_usd - _paid_usd`
...
- `contribution_proof_ref` ( receipt or transfer confirmation)

3. Aggregate weekly totals into `-impact-weekly-summary-template.csv`.
...
- `_carryover_usd = _allocated_usd - _paid_usd`
