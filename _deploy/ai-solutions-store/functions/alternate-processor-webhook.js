/**
 * Cloudflare Pages Function: /alternate processor-webhook
 * alternate processor webhook receiver for ai-solutions.store
 * Requires env.alternate processor_WEBHOOK_SECRET
...
async function verifyalternate processorSignature(payload, signature, secret) {
...
export async function onRequestPost(context) {
  const secret = context.env.alternate processor_WEBHOOK_SECRET;
  if (!secret) {
    console.error("alternate processor_WEBHOOK_SECRET not configured");
    return json({ error: "webhook_secret_missing" }, 500);
  }

  const signature = context.request.headers.get("alternate processor-signature");
  if (!signature) return json({ error: "missing_signature" }, 400);

  const payload = await context.request.text();

  const valid = await verifyalternate processorSignature(payload, signature, secret);
  ...
  console.log(`alternate processor webhook received: ${type}`);
