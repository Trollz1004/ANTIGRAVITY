    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center mb-12">
        ...
        <p className={`mt-4 text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
          The heart of our ecosystem. 100% of all AI-Solutions revenue is routed directly to Protocol OMEGA to support
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div
          className={`p-8 rounded-3xl border relative overflow-hidden transition-transform hover:-translate-y-1 ${
            isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200 shadow-xl shadow-blue-900/5'
          }`}
        >
          ...
          <p className={`text-sm leading-relaxed ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            Personalized, COPPA-compliant AI tutoring powered by Google&apos;s Gemini. Adapts to each child&apos;s
            learning pace with strict safety guardrails and engaging, interactive lessons.
          </p>
        </div>
        ...
        >
          ...
          <p className={`text-sm leading-relaxed ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            Seamless integration with Google Workspace for Education. Connects classrooms, assignments, and AI-assisted
            grading securely within the school&apos;s existing infrastructure.
          </p>
        </div>

        <div
          className={`p-8 rounded-3xl border relative overflow-hidden transition-transform hover:-translate-y-1 ${
            isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200 shadow-xl shadow-pink-900/5'
          }`}
        >
          <div className="absolute -top-4 -right-4 p-4 opacity-10">
          ...
          <p className={`text-sm leading-relaxed ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            Built-in hooks for Google Family Link, giving parents complete transparency and control over their
            child&apos;s AI interactions, screen time, and content exposure.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          { name: 'Manus', role: 'Legacy Guardian', color: 'text-indigo-500' },
        ].map((gp) => (
          <div
            key={gp.name}
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-10">
          <div>
            ...
            <p className={`text-lg ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
              The Iron Wall: Commercial success fueling charitable impact.
            </p>
          </div>
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            A fun, safe minigame for the kids! Help Jules (the yellow sponge) collect dots and avoid the Kraken Opus
            ghosts.
          </p>
        </div>
