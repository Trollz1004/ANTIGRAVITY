...
import { type DaoLaunch, type KickstarterProject, type StartupKpis, type View, type AuditLog, type SystemHealth } from './types';
...
const App: React.FC = () => {
    ...
    const handleAuditAction = (log: AuditLog) => {
        if (log.metadata?.actionType === 'security') {
            if (confirm(`Are you sure you want to LOCK account for ${log.metadata.targetId}?`)) {
             ...
             logAction(
                'Retry Operation', 
                `Retrying operation: ${log.action}`, 
                'pending',
                { ...log.metadata, actionType: 'none' }
            );
            // Simulate retry delay
            setTimeout(() => {
                logAction(
                    'Operation Success', 
                    `Successfully synced ${log.metadata?.targetId}`, 
                    'success'
                );
    ...
    const handleAnalyze = async (item: DaoLaunch | KickstarterProject | 'kpis' | 'daos') => {
        ...
        if (item === 'kpis') {
            ...
            prompt = `Analyze the following DAO launch data and provide a summary of key trends. Data: ${JSON.stringify(daoLaunches)}`;
        } else {
    ...
    const kpiCards = useMemo(() => startupKpis ? [
        { title: 'Monthly Recurring Revenue', value: `$${(startupKpis.mrr / 1000).toFixed(1)}k`, Icon: DollarSign, change: '+0.0%', changeType: 'positive', theme: 'revenue' },
        { title: 'Customer Acquisition Cost', value: `$${startupKpis.cac}`, Icon: Users, change: '0.0%', changeType: 'positive', theme: 'users' },
        { title: 'Churn Rate', value: startupKpis.churnRate, Icon: TrendingDown, change: '0.0%', changeType: 'negative', theme: 'matches' },
        { title: 'Monthly Burn Rate', value: `$${(startupKpis.burnRate / 1000).toFixed(0)}k`, Icon: Rocket, change: '0.0%', changeType: 'positive', theme: 'engagement' },
    ] as const : [
        // Zero State Cards
        { title: 'Monthly Recurring Revenue', value: `$0.00`, Icon: DollarSign, change: '--', changeType: 'positive', theme: 'revenue' },
        { title: 'Customer Acquisition Cost', value: `$0.00`, Icon: Users, change: '--', changeType: 'positive', theme: 'users' },
        { title: 'Churn Rate', value: '0%', Icon: TrendingDown, change: '--', changeType: 'negative', theme: 'matches' },
        { title: 'Monthly Burn Rate', value: `$0.00`, Icon: Rocket, change: '--', changeType: 'positive', theme: 'engagement' },
        ...
        if (view === 'command') {
            return (
                <div className="flex flex-col h-full overflow-hidden">
                    <CommandCenter onLogAction={logAction} repoStatus={repoStatus} onCheckStatus={handleCheckRepoStatus} systemHealth={systemHealth} />
            ...
            return (
                <main className="flex-1 p-4 md:p-6 lg:p-8 overflow-y-auto">
                     <header className="glass-card p-4 md:p-6 mb-8 flex justify-between items-center">
                        ...
                        <button onClick={() => setSidebarOpen(true)} className="md:hidden text-slate-400 hover:text-white"><Menu className="w-6 h-6" /></button>
                    </header>
                    <div className="max-w-7xl mx-auto">
                        <AuditLogTable logs={auditLogs} onAction={handleAuditAction} onExport={handleExportLogs} onImport={handleImportLogs} />
                    </div>
                </main>
            );
        }
            ...
            <main className="flex-1 p-4 md:p-6 lg:p-8 overflow-y-auto">
                 <header className="glass-card p-4 md:p-6 mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400">
                        <div className={`flex items-center gap-2 px-3 py-1 rounded-full border transition-colors ${backendUplink ? 'bg-green-900/20 border-green-500/30 text-green-400' : 'bg-red-900/20 border-red-500/30 text-red-400 animate-pulse'}`}>
                            <Server className="w-4 h-4" />
                            <span className="font-semibold">{backendUplink ? 'Empire Uplink: Active' : 'Empire Uplink: Offline'}</span>
                        </div>

                        <div className={`flex items-center gap-2 px-3 py-1 rounded-full border transition-colors ${systemHealth === 'Healthy' ? 'bg-green-900/20 border-green-500/30 text-green-400' : systemHealth === 'Degraded' ? 'bg-yellow-900/20 border-yellow-500/30 text-yellow-400' : 'bg-red-900/20 border-red-500/30 text-red-400 animate-pulse'}`}>
                         ...
                         <div className={`flex items-center gap-2 px-3 py-1 rounded-full border ${repoStatus === 'Synced' ? 'bg-green-500/10 border-green-500/20 text-green-400' : repoStatus === 'Checking' ? 'bg-blue-500/10 border-blue-500/20 text-blue-400' : repoStatus === 'Behind' ? 'bg-yellow-500/10 border-yellow-500/20 text-yellow-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
                            <button onClick={handleCheckRepoStatus} className="flex items-center gap-2 focus:outline-none hover:bg-white/5 rounded-full px-2 -ml-2 transition-colors cursor-pointer" title="Click to Check Sync Status">
                                <div className={`w-2 h-2 rounded-full ${repoStatus === 'Synced' ? 'bg-green-400' : repoStatus === 'Checking' ? 'bg-blue-400 animate-ping' : repoStatus === 'Behind' ? 'bg-yellow-400' : 'bg-red-400'}`} />
                ...
                <div className="max-w-7xl mx-auto">
                    {view === 'dashboard' && (
                        <>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                                {loading ? Array.from({ length: 4 }).map((_, i) => <StatCardSkeleton key={i} />) : kpiCards.map(kpi => <StatCard key={kpi.title} {...kpi} />)}
                            </div>
                            {/* Show tables with loading state or if data exists */}
                            {(loading || daoLaunches.length > 0) && (
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    <div className="lg:col-span-2">
                                        <DaoTable daoLaunches={daoLaunches} onAnalyze={handleAnalyze} onAnalyzeTrends={() => handleAnalyze('daos')} isLoading={loading} />
                                    </div>
                                    <div><KickstarterTable projects={kickstarterProjects} isLoading={loading} /></div>
                                </div>
                            )}
                            {!loading && daoLaunches.length === 0 && (
                                <div className="glass-card p-12 text-center border border-dashed border-white/10">
                                    <div className="text-slate-500 mb-2"><Rocket className="w-12 h-12 mx-auto opacity-20"/></div>
                    ...
                    {view === 'dao' && (
                        <div>
                            {loading ? <TableSkeleton /> : <DaoTable daoLaunches={daoLaunches} onAdd={() => setAddDaoModalOpen(true)} onAnalyze={handleAnalyze} onAnalyzeTrends={() => handleAnalyze('daos')} />}
                        </div>
                    )}

                    {view === 'kickstarter' && (
                        <>
                            {!loading && <KickstarterFilters nameFilter={kickstarterNameFilter} onNameChange={setKickstarterNameFilter} goalFilter={kickstarterGoalFilter} onGoalChange={setKickstarterGoalFilter} onClear={handleClearKickstarterFilters} />}
                            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                                <div className="lg:col-span-3">{loading ? <ChartSkeleton /> : <KickstarterChart projects={filteredKickstarterProjects} />}</div>
                                <div className="lg:col-span-2">{loading ? <KickstarterListSkeleton /> : <KickstarterTable projects={filteredKickstarterProjects} onAnalyze={handleAnalyze} />}</div>
                            </div>
                        </>
                    )}
                </div>
    ...
    return (
        <div className="flex flex-col h-screen bg-dark-bg text-slate-200 font-sans">
            ...
            <div className="flex flex-1 overflow-hidden pt-[30px]">
                <Sidebar currentView={view} onSetView={handleSetView} isMobileOpen={isSidebarOpen} onMobileClose={() => setSidebarOpen(false)} repoStatus={repoStatus} onOpenMission={() => setIsMissionOpen(true)} />
                <div className="flex-1 flex flex-col overflow-hidden">{renderContent()}</div>
            </div>
            {isAddDaoModalOpen && <AddDaoModal onClose={() => setAddDaoModalOpen(false)} onAdd={handleAddDao} />}
            {isAnalysisModalOpen && <AnalysisModal isOpen={isAnalysisModalOpen} onClose={() => setAnalysisModalOpen(false)} title={analysisTitle} content={analysisContent} isLoading={isAnalyzing} />}
