...
- **Gemini CLI Launch Path:** In the Codex embedded PowerShell terminal, launch Gemini with `& 'C:\ANTIGRAVITY\scripts\Start-Gemini-Clean.ps1'`. In the ANTIGRAVITY admin startup shell, use the `gemini` helper from `scripts/startup-pwsh-admin.ps1`. Gemini CLI can fail if it inherits the `\\?\` provider-qualified path, so both entrypoints force a normal `C:\ANTIGRAVITY` cwd.
