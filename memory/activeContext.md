...
- **T5500 (`192.168.0.15` / `DESKTOP-H4B53GL`):** SSH responds and the candidate scan re-confirmed `.15` as the live T5500 IP. A stray manual `node.exe` had been polling Telegram on `127.0.0.1:18789`; Codex removed Telegram config there, killed that extra process, repaired the broken `OpenClaw Gateway` scheduled task, and verified the listener is back on loopback only (`127.0.0.1:18789` / `[::1]:18789`).
...
5. **Keep OpenClaw local-only:** `gateway.mode=local` is the enforced posture. Verified listeners on Sabretooth, 9020, and T5500 are loopback-only, not LAN-bound.
