...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, label, error, hint, id, ...props }, ref) => {
    ...
    return (
      <div className="w-full">
        {label && (
          <label htmlFor={textareaId} className="block text-sm font-medium text-nexus-700 dark:text-nexus-300 mb-1.5">
            {label}
          </label>
        )}
        <textarea
        ...
        {hint && !error && (
          <p id={`${textareaId}-hint`} className="mt-1.5 text-sm text-nexus-500 dark:text-nexus-400">
            {hint}
          </p>
        )}
      </div>
