...
function RoutingDiagram({ hermesOn }) {
  // animated SVG dots flowing along the routes
  return (
    <svg viewBox="0 0 720 360" style={{ width: '100%', height: 'auto', display: 'block' }}>
      ...
      <g>
        ...
        <rect x="296" y="165" width="128" height="60" rx="18" fill={hermesOn ? '#2a1235' : '#1a0e26'} stroke={hermesOn ? '#b3a2bf' : 'rgba(255,255,255,0.18)'} strokeWidth="1.5"/>
        ...
        {hermesOn && <circle cx="312" cy="180" r="3" fill="#3fbf72"><animate attributeName="opacity" values="1;0.3;1" dur="1.6s" repeatCount="indefinite"/></circle>}
      </g>
      ...
      {hermesOn && [
        ...
        <circle key={'p' + i} r="3" fill="#b3a2bf">
      ...
      {hermesOn && [
        'M 180 60 C 240 60, 270 195, 296 195',
        'M 180 150 C 240 150, 270 195, 296 195',
        'M 180 240 C 240 240, 270 195, 296 195',
        'M 424 195 C 460 195, 480 50, 540 50',
        'M 424 195 C 460 195, 480 130, 540 130',
        'M 424 195 C 460 195, 480 210, 540 210',
      ].map((d, i) => <path key={'pp' + i} id={`path-${i}`} d={d} fill="none" stroke="none"/>)}
    </svg>
  );
}

function HermesNode({ hermesOn, onToggle }) {
  return (
      ...
      <div className="bento" style={{ marginTop: 22 }}>
        <div className="card span-4">
          <div className="card-h">
            ...
            <span className={"tag " + (hermesOn ? 'live' : 'idle')}><span className={"status-dot " + (hermesOn ? 'live' : 'idle')}/> {hermesOn ? 'LIVE' : 'IDLE'}</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div className="bar-row green">
              <span className="label">CPU</span>
              <div className="bar"><i style={{ width: hermesOn ? '38%' : '6%' }}/></div>
              <span className="v">{hermesOn ? '38%' : '6%'}</span>
            </div>
            <div className="bar-row sky">
              <span className="label">VRAM</span>
              <div className="bar"><i style={{ width: hermesOn ? '78%' : '14%' }}/></div>
              <span className="v">{hermesOn ? '78%' : '14%'}</span>
            </div>
            <div className="bar-row amber">
              <span className="label">CONTEXT</span>
              <div className="bar"><i style={{ width: hermesOn ? '52%' : '2%' }}/></div>
              <span className="v">{hermesOn ? '52%' : '2%'}</span>
            </div>
            <div className="bar-row">
              <span className="label">HOPS / S</span>
              <div className="bar"><i style={{ width: hermesOn ? '88%' : '4%' }}/></div>
              <span className="v">{hermesOn ? '12.4' : '0.0'}</span>
            </div>
            <div className="bar-row">
              <span className="label">QUEUE</span>
              <div className="bar"><i style={{ width: hermesOn ? '22%' : '0%' }}/></div>
              <span className="v">{hermesOn ? '3' : '0'}</span>
            </div>
          </div>
        </div>
