...
const SEED_MSGS: MsgItem[] = [
  { kind: 'system', body: '— TODAY · MAY 12, 2026 —' },
  {
    id: 'm1', who: 'opus', name: 'Opus', role: 'Chief of Staff', ts: '11:32',
    body: <>Initializing team sync. Routing all chat through <code>9020/openclaw</code>.</>,
  },
  {
    id: 'm2', who: 'hermes', name: 'Hermes', role: 'Router · 9020', ts: '11:33', bridge: true,
    body: <>API server listening. <code>localhost:11435</code> · transparent mode <strong>ON</strong>. All inputs/outputs broadcast here.</>,
    attach: [
  ...
  },
  {
    ...
    body: <>Acknowledged. Monitoring channel for context injection. Cross-referencing <code>~/memory/semantic</code> for prior decisions.</>,
  },
  {
    id: 'm4', who: 'josh', name: 'Joshua Coleman', role: 'Operator', ts: '11:38',
    body: <>quick check — can the fleet absorb a 2-hour spike on YouAndINotAI moderation tonight?</>,
  },
  {
    id: 'm5', who: 'opus', name: 'Opus', role: 'Chief of Staff', ts: '11:39',
    body: <>Yes. Pre-warming Gemma4 + Pi for moderation overflow. Drafting sticky <code>PAPA-241</code>.</>,
  },
  {
    id: 'm6', who: 'hermes', name: 'Hermes', role: 'Router · 9020', ts: '11:40', bridge: true,
    body: <>Sticky PAPA-241 dispatched · assigned to <strong>Opus</strong> · ETA 8m.</>,
  },
  {
    id: 'm7', who: 'gemini', name: 'Gemini', role: 'AI Strategy · 2.5', ts: '11:41',
    body: <>Drafted moderation playbook v2. Three branches: (1) auto-allow verified accounts, (2) soft-quarantine ad origins, (3) escalate ambiguous to Opus. Attach to sticky?</>,
  },
];

export function CommsGateway() {
  const [activeId, setActiveId] = useState('mission');
  const [msgs, setMsgs] = useState<MsgItem[]>(SEED_MSGS);
  ...
  const send = () => {
    ...
    const me: MsgItem = { id: 'u' + Date.now(), who: 'josh', name: 'Joshua Coleman', role: 'Operator', ts, body: input };
    setMsgs(prev => [...prev, me]);
    const captured = input;
    setInput('');

    setTimeout(() => {
      const txt = captured.toLowerCase();
      let r: Partial<MsgItem>;
      if (txt.includes('hermes') || txt.includes('run')) {
        ...
        r = { who: 'opus', name: 'Opus', role: 'Chief of Staff', body: <>Logged. Persisting to <code>~/memory/working</code> and promoting if it sticks.</> };
      }
      const now2 = new Date();
      const ts2 = `${String(now2.getHours()).padStart(2,'0')}:${String(now2.getMinutes()).padStart(2,'0')}`;
      setMsgs(prev => [...prev, { id: 'a' + Date.now(), ts: ts2, ...r } as MsgItem]);
    }, 900);
  };
          ...
          <div>
            ...
            <div className="comms-channels">
              {CHANNELS.map(c => (
                ...
                >
                  <Icon.Hash size={14} style={{ color: c.tag === 'sky' ? 'var(--sky)' : c.tag === 'indigo' ? '#8d7fd4' : 'var(--ink-3)' }}/>
                  <span>{c.name}</span>
        ...
        <div className="comms-thread">
          ...
          <div className="thread-stream" ref={scrollRef}>
            {msgs.map((m, i) => {
              ...
              return (
                <div key={m.id ?? i} className={'msg ' + (m.bridge ? 'bridge' : '')}>
          ...
          <div className="composer">
            ...
            <div className="composer-input">
              <button className="ico-btn" aria-label="Attach file"><Icon.Attach size={16}/></button>
              <input
                value={input}
              ...
              />
              <button className="ico-btn" aria-label="Voice input"><Icon.Mic size={16}/></button>
              <button className="send" onClick={send} aria-label="Send message"><Icon.Send size={15}/></button>
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                { from: '@_partner',msg: '"sponsorship deck draft attached"', when: '1h' },
              ].map((it, i) => (
