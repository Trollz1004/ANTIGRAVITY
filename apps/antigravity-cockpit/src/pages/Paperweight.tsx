...
interface StickyData {
  id: string;
  color: 'yellow' | 'pink' | 'green' | 'blue' | 'violet' | 'orange';
  x: number;
  y: number;
  rot: number;
  agent: string;
...
const STICKIES: StickyData[] = [
  { id: 'PAPA-241', color: 'yellow', x: 32,  y: 24,  rot: -3.2, agent: 'OPUS',   status: 'progress', stateLabel: 'IN PROGRESS', task: 'Audit task router for race conditions before tonight\'s ad spike.', age: '8m',  from: 'JC' },
  ...
  { id: 'PAPA-238', color: 'blue',   x: 92,  y: 240, rot: 1.8,  agent: 'CODEX',  status: 'progress', stateLabel: 'IN PROGRESS', task: 'pnpm test --filter runtime · then commit the regen.', age: '1h',  from: 'OPUS' },
...
function Sticky({ s, mode }: { s: StickyData; mode: 'cork' | 'kanban' }) {
  const style: React.CSSProperties = mode === 'cork'
...
export function Paperweight() {
  const [mode, setMode] = useState<'cork' | 'kanban'>('cork');

  const lanes = [
    { id: 'inbox',     label: 'INBOX',       items: STICKIES.filter(s => s.status === 'pending' && s.from === 'JC') },
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              ...
              <select style={{ width: '100%', padding: '10px 12px', borderRadius: 12, background: 'var(--bg-3)', border: '1px solid var(--line-2)', color: 'var(--ink)', fontFamily: 'Inter, sans-serif', fontSize: 13, outline: 'none' }}>
                <option>Opus · Chief of Staff</option>
                <option>Gemini · AI Strategy</option>
