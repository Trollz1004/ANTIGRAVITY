...
| `fable audit` | Reads `mission-control/config/sentry-targets.json` and probes every target exactly the way Fable's Sentry does (port / http+identity / redis PING — a port answering is never reported UP by itself). Also fetches JARVIS's own live `http://127.0.0.1:9150/api/sentry` and diffs against it — Fable's Sentry stopped being a separate `:9140` service 2026-09-18; the probe engine now lives inside JARVIS (`mission-control/lib/sentry.mjs`). Verdicts: `UP · DOWN · WRONG SERVICE · AUTH MISSING · AUTH REJECTED · NOT CONFIGURED`. |
