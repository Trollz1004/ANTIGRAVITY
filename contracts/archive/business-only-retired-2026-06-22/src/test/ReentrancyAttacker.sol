 ...
 * @notice Test-only contract used to verify that Router100's ETH
 *         forwarding path cannot be exploited by a malicious recipient.
 *
 *         Because Router100 has an immutable _SAFE, this
 *         contract can never become the recipient — the test uses it to
 *         confirm that ETH goes only to the legitimate  safe and
...
contract ReentrancyAttacker {
    ...
    ///      _SAFE is immutable and points elsewhere.
    receive() external payable {
        attackCount++;
        if (attackCount < 3) {
            // Attempt to trigger another distribution — will fail or be irrelevant
            // because this contract can never be _SAFE
