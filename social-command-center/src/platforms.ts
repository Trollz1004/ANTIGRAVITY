...
export const PLATFORMS: Record<string, PlatformEntry> = {
  ...
  youtube: {
    ...
    corsSupport: "browser",
    notes: "Browser-direct via cors_upload.js + GIS. Intermittent CORS failures on upload endpoint — build server fallback.",
  },
  instagram: {
    id: "instagram",
    label: "Instagram",
  ...
  facebook: {
    ...
    api: "Meta Graph API",
    corsSupport: "browser",
    notes: "FB JS SDK v22.0+ for Pages only. Personal timeline posting deprecated 2018. Long-lived token exchange needs secret server-side.",
  },
  tiktok: {
    id: "tiktok",
    label: "TikTok",
    ...
    corsSupport: "backend-proxy",
    notes: "Most restrictive. Server-only API. 3+ sequential API calls. Tokens expire every 24h. Unaudited = SELF_ONLY visibility.",
  },
  twitter: {
    id: "twitter",
    label: "X / Twitter",
    ...
    api: "X API v2",
    corsSupport: "backend-proxy",
    notes: "No CORS headers on any endpoint. PKCE possible but useless — fetch dies at preflight. Server proxy mandatory.",
  },
  linkedin: {
    id: "linkedin",
    label: "LinkedIn",
  ...
  reddit: {
    ...
    api: "Reddit API v1",
    corsSupport: "backend-proxy",
    notes: "oauth.reddit.com sends no CORS headers. snoowrap non-functional in browsers. Implicit tokens 1h, no refresh.",
  },
  pinterest: {
    id: "pinterest",
    label: "Pinterest",
