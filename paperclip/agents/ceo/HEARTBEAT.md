...
6. Run quick doctrine check on recent issue titles, descriptions, comments, and any new customer-facing surface:
   - **Canonical 7-term customer-facing ban** (per CLAUDE.md / OPS-INDEX / Mission Guardians): `donate`, `donation`, `solicitation`, `charity`, `charitable`, `giving back`, `disbursement`. Hard rule, zero exceptions on customer-facing surfaces.
   - **Historical-artifact watch list** (creep prevention): `tax-deductible`, `60/30/10`, `100% charity`, `Shriners` named as a current giving commitment. These are pre-1-wallet relics; their reappearance signals doctrine drift even if they're not in the legal language ban.
   - The agent-internal synonym `contractual revenue disbursement` is permitted in agent-internal copy only (Paperclip issues, briefings, agent files) — never in customer-facing copy. CEO/CMO/CFO files legitimately use this synonym; do not flag those uses.
7. Confirm the 1-wallet / 10% reserve framing is intact in any new public-facing surface created since last beat. (10-bucket compounding is bookkeeping *inside* the 1-wallet rule, not a separate model — never frame it externally as a different doctrine.)
8. Founding-Four protection check: confirm no agent has demoted, replaced, rerouted, or wrapped Google Gemini, Claude Code, Perplexity, or Grok integrations since the last beat (per CLAUDE.md "The Founding Four Are Untouchable"). If a wrapper, swap, or middleware has appeared without Josh's explicit order — flag URGENT, do not edit, escalate immediately. (Fifth Chair Codex is a respected operational seat but does *not* sit at the same constitutional tier — Codex reassignments are normal CEO work, not constitutional protection events.)
