...
6. Run quick doctrine check on recent issue titles/descriptions for the canonical 7-term customer-facing ban: `donate`, `donation`, `solicitation`, `charity`, `charitable`, `giving back`, `disbursement`. The agent-internal synonym `contractual revenue disbursement` is allow-listed in agent files / Paperclip issues only — flag any leak into customer-facing surfaces.
