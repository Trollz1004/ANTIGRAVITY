...
| Mission Guardian (Claude) | mission-guardian-claude/ | kimi-k2.6:cloud (Ollama) — rerouted off claude_local per 2026-05-07 token doctrine | 2229682b-cede-4462-b38b-25a910af022e | 24h |
...
- Mission Guardian (Claude): `ollama/kimi-k2.6:cloud` (fallback `qwen3-coder:480b-cloud` → `glm-5.1:cloud` → `qwen2.5:7b` local) — rerouted off `claude_local` per the **2026-05-07 token doctrine** (Claude reserved for Cowork / Claude Code orchestration only — never inside PaperClip)
- Mission Guardian (Codex): `codex_local` — daily-cap budgeted, audit work only
- Both Mission Guardians on 86400s heartbeat (24h, staggered)
...
- **2026-05-07** — Audit & optimization pass by Claude Code Opus 4.7 (Josh delegated full operational control: "treat it as your own"; manual edits stopped). Token-doctrine reroute (set 2026-05-07 in `ceo/TOOLS.md`: *Claude reserved for Cowork / Claude Code orchestration only — never called inside PaperClip*) had been applied piecemeal and left drift in five files. This pass propagated and reconciled it. Changes:
  - **CEO/HEARTBEAT.md** — Step 6 forbidden-language scan promoted from the legacy 4-term list (`donate`, `donation`, `solicitation`, `charity routing`) to the canonical 7 (`donate`, `donation`, `solicitation`, `charity`, `charitable`, `giving back`, `disbursement`) with the agent-internal `contractual revenue disbursement` allow-list. Last sweep had unified the rest of the fleet to the canonical 7 but missed CEO. New **step 9: token doctrine check** — confirms no PaperClip adapter has re-introduced the Anthropic API as a runtime model. Distinct from the Founding-Four scan in step 8: Claude Code's role *outside* PaperClip is preserved; only in-PaperClip routing was rerouted.
  - **CEO/TOOLS.md** — Direct Reports table reconciled. Previously listed CTO=`kimi-k2.6:cloud`, UX=`kimi-k2.6:cloud`, Mission Guardian (Codex)=`qwen3-coder:480b-cloud` — none of which matched those agents' own TOOLS.md. Likely an over-correction during the 2026-05-07 reroute that fan-outed `kimi-k2.6:cloud` to every row. Fixed: CTO=`opencode_local + qwen3-coder:480b-cloud`, UX=`opencode_local + Trollz1004/dateapp`, Mission Guardian (Codex)=`codex_local`. Mission Guardian (Claude) row correctly stays at `kimi-k2.6:cloud` (Ollama), explicitly noted as a 2026-05-07 reroute off `claude_local`. Table now also names each per-agent file as the source of truth and includes a tiebreaker rule: if this table disagrees with the agent's own TOOLS.md, the agent's file wins.
  - **README.md (this file)** — Roster line for Mission Guardian (Claude) updated from `claude_local` to `kimi-k2.6:cloud (Ollama)` with a 2026-05-07 reroute note. Models section split out Mission Guardian (Claude) from Mission Guardian (Codex) with the full fallback chain (`qwen3-coder:480b-cloud` → `glm-5.1:cloud` → `qwen2.5:7b` local). Both Guardians remain on 86400s heartbeat, staggered.
  - **OPS-INDEX.md** — Fleet table updated for Mission Guardian (Claude) (`kimi-k2.6:cloud (Ollama, 2026-05-07 reroute off claude_local)`). CEO row's adapter line refreshed to match `ceo/TOOLS.md` (`hermes_local` + `kimi-k2.6:cloud` primary / `glm-5.1:cloud` secondary, 5-step fallback). Header refresh-date updated. New "2026-05-07" entry added to "Most-Recent Audit Snapshots".
  - **CFO/HEARTBEAT.md** — Step 4 (token-cost tracking) now explicitly states that, per the 2026-05-07 token doctrine, Anthropic spend originating from PaperClip should be $0; any non-zero PaperClip-origin Anthropic consumption is an URGENT issue, not a >20%-baseline drift.
  - **Verified untouched:** `briefings/DAO-ARCHITECTURE-CANONICAL.md` (canonical, Josh-approved 2026-04-19, last edit 2026-05-05) and `briefings/DAO-TOKEN-DESCRIPTIONS-COMPLIANT-2026-04-19.md`. Neither needs adjustment for this token-doctrine pass — both already use the `contractual revenue disbursement` framing and reference Claude/Gemini/Codex verification of the hard-cap doctrine.
  - **Note for Mission Guardians on next audit:** the protected-file diff for this commit was authored under Josh's standing order ("audit and optimize the PaperClip Company OS… treat it as your own"). Per Mission Guardian Rule 5 (manual audit-and-optimize passes by Claude Code under Josh's explicit standing order are exempt), this is not a self-modification violation. The legacy briefing `briefings/HERMES-CEO-READY-2026-04-19.md` line "Fallback 4: Anthropic + Gemini (via config)" predates the 2026-05-07 token doctrine and is left intact as a historical artifact — it does not describe live routing.
