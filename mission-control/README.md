...
- **Phase C** — Social command center `GET/POST /api/social/*`, approval inbox `GET /api/inbox`. Brand scope: `DREAM Online` and `AI Solutions` post freely; `youandinotai` (aliases `YouAndINotAI`, "date app") was added 2026-09-19 — marketing unfrozen, features/checkout/listing stay frozen — and every youandinotai proposal must also clear the 18-and-over and business-only checks in `lib/social-adapters.mjs` before it is created. `POST /api/social/draft` drafts youandinotai copy in Joshua's Fable voice (Ollama model `joshlcoleman/Fable`) without ever creating a proposal.
