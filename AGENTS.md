...
| `scripts/` | `link-skills.ps1`, `sync-agents.ps1` (see above) |
...
**Skills and agents sync** (PowerShell, from the repo root):
```powershell
.\scripts\link-skills.ps1 -Check     # every repo skill is linked into C:\DREAM\.agents\skills
.\scripts\sync-agents.ps1 -Check     # every platform agent file matches its SKILL.md
```
