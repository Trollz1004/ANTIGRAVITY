...
- Claude Code plugins on this node (user scope, installed 2026-09-13 to mirror Sabertooth): `claude-obsidian@agricidaniel-claude-obsidian` (save, think, wiki-* skills, obsidian-markdown, obsidian-bases) and `supermemory@supermemory-plugins` (reads `SUPERMEMORY_CC_API_KEY`; the SDK and Hermes read `SUPERMEMORY_API_KEY`; both are set as user environment variables from the repo `.env`'s `SUPER_MEMORY_API_KEY`). The Obsidian MCP server `obsidian` (user scope) points at the vault's `/second-brain-mcp/` endpoint; it connects only once the Local REST API's plain-HTTP server (`:27123`) is enabled, because the HTTPS port uses a self-signed certificate. Its token lives in the gitignored `mcpServers.jsonl`.
