      ...
      <div ref={scrollRef} className="flex-1 overflow-auto p-6 space-y-6">
        {messages.length === 0 && (
          <div className="text-center py-20 text-slate-600 flex flex-col items-center">
            ...
            <p className="text-[11px] font-medium leading-relaxed max-w-[260px] mx-auto text-slate-500">
              Josh, scaling is the only metric. What's the play for today's pre-order velocity? Speak or type your
              command.
            </p>
          </div>
        )}
          ...
          <div key={i} className={`flex gap-4 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div
              className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center border ${
                m.role === 'user' ? 'bg-slate-800 border-slate-700' : 'bg-red-500 border-red-500'
              }`}
            >
              {m.role === 'user' ? (
            ...
            <div
              className={`max-w-[85%] p-4 rounded-2xl text-[13px] font-medium relative leading-relaxed shadow-xl ${
                m.role === 'user'
                  ? 'bg-slate-800 text-white rounded-tr-none border border-slate-700'
                  : 'bg-slate-900 border border-red-500/20 text-slate-200 rounded-tl-none'
              }`}
            >
              {m.text}
            </div>
        ...
        {isThinking && (
          <div className="flex gap-4">
            <div className="shrink-0 w-10 h-10 rounded-xl bg-red-500 flex items-center justify-center animate-pulse">
      ...
      <div className="p-6 bg-slate-950/80 border-t border-slate-800">
        <div className="relative">
          <input
            ...
            className={`w-full bg-slate-800 border-2 border-slate-700 rounded-2xl pl-6 pr-24 py-4 text-sm text-white focus:outline-none focus:border-red-500 transition-all font-black uppercase placeholder-slate-600 ${
              isRecording ? 'border-red-500 ring-4 ring-red-500/20' : ''
            }`}
          />
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
            ...
            <button
              onClick={handleSend}
              className="w-10 h-10 bg-red-500 hover:bg-red-600 text-white rounded-xl flex items-center justify-center transition-all shadow-lg shadow-red-500/30"
            >
              <Send size={18} />
            </button>
          </div>
