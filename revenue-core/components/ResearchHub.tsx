          ...
          <div className="p-6 bg-emerald-500/5 border-b border-emerald-500/20 animate-fade-in">
            <div className="flex justify-between items-start mb-4">
              <h3 className="font-bold text-emerald-400 flex items-center gap-2">
                <Globe size={18} /> Grounded AI Analysis
              </h3>
              <button onClick={() => setMapResults(null)} className="text-slate-500 hover:text-white">
                Close
              </button>
            </div>
            <p className="text-slate-300 text-sm leading-relaxed mb-4">{mapResults.text}</p>
            <div className="flex flex-wrap gap-2">
              {mapResults.chunks.map(
                (chunk: any, i: number) =>
                  chunk.maps?.uri && (
                    <a
                      key={i}
                      href={chunk.maps.uri}
                      target="_blank"
                      className="text-[10px] bg-slate-800 text-emerald-400 px-2 py-1 rounded border border-emerald-500/30 flex items-center gap-1"
                    >
                ...
                <tr key={item.id} className="hover:bg-slate-700/30 transition-colors group">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="p-2 bg-slate-800 hover:bg-slate-700 rounded text-slate-300">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
