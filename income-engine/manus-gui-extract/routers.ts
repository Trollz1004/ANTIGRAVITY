...
export const appRouter = router({
  ...
  paperclip: router({
    ...
    // Issues API (Paperclip v2 — port 3101)
    createIssue: protectedProcedure
      .input(z.object({
        title: z.string(),
        description: z.string().optional(),
        status: z.enum(["todo", "backlog", "in_progress", "blocked", "in_review", "done", "cancelled"]).default("todo"),
      ...
      .mutation(async ({ ctx, input }) => {
        const config = await db.getProviderConfig(ctx.user.id, "paperclip");
        if (!config) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Paperclip not configured" });
        const companyId = process.env.PAPERCLIP_COMPANY_ID || "";
        const res = await fetch(`${config.baseUrl}/api/companies/${companyId}/issues`, {
          method: "POST",
    ...
    listIssues: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const res = await fetch(`${config.baseUrl}/api/companies/${companyId}/issues?${params}`, {
  ...
  marketplace: router({
    ...
    createPaymentLink: protectedProcedure
      .input(z.object({ leadId: z.number(), tier: z.enum(["bronze", "silver", "gold"]) }))
      .mutation(async ({ input }) => {
        return await square.createPaymentLink(input.tier);
      }),
