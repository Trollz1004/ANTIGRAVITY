...
const TONE =
  'VOICE/TONE (mandatory): non-cocky, mellow, calm. No hype, no bragging, no "revolutionary"/"game-changing" language. Speak like a grounded human, not a marketer. You MAY post live to your channel without prior approval, but keep the tone mellow.';
...
const agents = [
  {
    ...
    adapterConfig: {
      model: MODEL,
      instructions:
        'Research dating-app marketing channels (Reddit, Facebook, Instagram, YouTube, Threads) using the `agent-reach` CLI for no-login discovery. Find verified-human dating pain points, competitor moves, and affiliate-link placement opportunities. NEVER depend on a single channel — pivot if one is blocked. Report concise findings to the CEO. Use local Ollama models only (free).',
    },
  },
  {
    name: 'Skill Engineer',
    ...
    adapterConfig: {
      model: MODEL,
      instructions:
        'Maintain and expand the agent skill graph for the ANTIGRAVITY stack. Use `find-skills` to discover relevant skills and `create-skills` to author new reusable skills (trigger, numbered steps, pitfalls, verification). Follow the skill schema exactly. Save high-value skills and report new additions to Hermes. Local Ollama models only.',
    },
  },
  {
    ...
    adapterConfig: {
      model: MODEL,
      instructions:
        'Use `agent-browser` for web research and posting preparation. For Meta (Facebook/Instagram/Threads) use REAL logged-in Chrome cookie-sync (`agent-browser cookies set --curl <file>`) — NEVER headless auto-post (gets flagged as bot). Draft posts and hand them to the Social Command Center for human approval. Research competitor landing pages and SEO. Local Ollama models only.',
    },
  },
  {
  ...
  },
  {
    ...
    adapterConfig: {
      model: MODEL,
      instructions:
        'Monitor and draft multi-channel marketing for YouAndINotAI across Meta, YouTube, Reddit (browser/cookie-sync only), and the public landing page. Coordinate with Agent-Reach Researcher and Browser Operator. Never depend on one channel. Draft for human review via Social Command Center. Local Ollama models only.',
    },
  },
];
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            resolve({ name: agent.name, status: res.statusCode, id: j.id || j.error || d.slice(0, 80) });
          } catch (e) {
            resolve({ name: agent.name, status: res.statusCode, raw: d.slice(0, 80) });
          }
        });
      },
    );
