...
export function Boards() {
  ...
  const loadComments = async (post: PostData) => {
    ...
    const data = await api.get<CommentData[]>(
      `/boards/${post.board_slug}/posts/${post.id}/comments`
    );
    setComments(data);
  };
  ...
  const submitPost = async () => {
    ...
    await api.post(`/boards/${activeBoard}/posts`, {
      title: newTitle,
      body: newBody,
    });
    setNewTitle('');
  ...
  const submitComment = async () => {
    ...
    await api.post(
      `/boards/${activePost.board_slug}/posts/${activePost.id}/comments`,
      { body: newComment }
    );
    setNewComment('');
  ...
  const reportActivePost = async () => {
    ...
    const reason = window.prompt(
      'Brief reason for reporting this post',
    ...
    return (
      <div className="app-page animate-fade-in">
        <button
          ...
          className="app-back-link mb-6 group"
        >
          <ArrowLeft
            size={18}
        ...
        <div className="glass-strong rounded-3xl p-6 glass-highlight mb-8">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3 text-xs text-gray-500">
              <span className="glass rounded-full px-3 py-1 font-medium">
        ...
        <div className="space-y-3 mb-6 stagger-children">
          {comments.map(c => (
            <div
              key={c.id}
              className="glass rounded-2xl p-4 hover:bg-white/[0.03] transition-colors"
            >
        ...
        {safetyAvailable && (
          <SafetyDrawer
            ...
            onBlocked={() => {
              setBoardNotice(
                `${activePost.author_name} was blocked and removed from your board lane.`
              );
              setSafetyOpen(false);
              setActivePost(null);
              if (activeBoard) {
      ...
      <div className="app-page animate-fade-in">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-black tracking-[-0.06em] text-[#111111]">
              {boardInfo?.name}
            </h1>
          </div>
          <button
    ...
    <div className="app-page">
      <div className="animate-fade-in">
        ...
        <p className="app-subtitle mb-6">
          Connect, share updates, and keep the app social without losing the
          main dating flow.
        </p>
      </div>
