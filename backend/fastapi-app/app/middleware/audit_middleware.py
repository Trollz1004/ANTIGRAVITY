...
class AuditMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if request.method == "GET" or request.url.path.startswith(('/static/', '/docs', '/redoc')):
            response = await call_next(request)
            return response

        # Capture request details before processing
        user_id = request.headers.get("X-User-ID", "anonymous") # Or extract from JWT/session
        ...
        try:
            ...
            request_body = {"error": "Could not decode JSON body"}
        ...
        if response.status_code < 400 and response.headers.get("content-type") == "application/json":
            try:
                ...
                response.body_iterator = AsyncIteratorWrapper([response_body_bytes]) # Restore body for subsequent readers
