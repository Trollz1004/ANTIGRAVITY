...
class AffiliateLink(Base):
    """An affiliate link generated by the Link Forge for tracking referrals."""
...
class AffiliateConversion(Base):
    """A conversion (registration, subscription, etc.) attributed to an affiliate click."""
    ...
    click_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("affiliate_clicks.id", ondelete="CASCADE"), nullable=False, index=True
    )
    link_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("affiliate_links.id", ondelete="CASCADE"), nullable=False, index=True
    )
