...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "source", sa.String(length=50), server_default="square", nullable=False
        ),
        ...
        sa.Column(
            "charitable_percent", sa.Integer(), server_default="10", nullable=False
        ),
        sa.Column(
            "beneficiary_lane",
            sa.String(length=100),
            server_default="kids_support",
            nullable=False,
        ),
        sa.Column(
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_revenue_allocations_square_payment_id", table_name="revenue_allocations"
    )
    op.drop_index(
        "ix_revenue_allocations_source_event_id", table_name="revenue_allocations"
    )
