...
def test_video_ws_invalid_token_rejected(client):
    ...
    with pytest.raises(Exception):  # noqa: B017
        ...
        with client.websocket_connect(
            f"/api/v1/ws/video/{call_id}?token=bad.membership_record.here"
        ) as ws:
            ws.receive_json()
...
def test_video_ws_non_uuid_call_id_rejected(client):
    ...
    membership_record = _valid_token(user)
    with pytest.raises(Exception):  # noqa: B017
        with client.websocket_connect(
            f"/api/v1/ws/video/not-a-uuid?token={membership_record}"
        ) as ws:
            ws.receive_json()
...
def test_video_ws_connects_with_valid_token_and_call_id(client, db_session_factory):
    ...
    with patch(
        ...
        with patch(
            ...
            with client.websocket_connect(
                f"/api/v1/ws/video/{call_id}?token={membership_record}"
            ) as ws:
