...
from app.revenue_allocation import calculate_charitable_amount_cents, reserve_revenue_allocation
...
async def test_reserve_revenue_allocation_success(db_session_factory):
    async with db_session_factory() as db:
        ...
        # Verify persistence
        result = await db.scalar(select(RevenueAllocation).where(RevenueAllocation.source_event_id == source_event_id))
...
async def test_reserve_revenue_allocation_invalid_inputs(db_session_factory):
    async with db_session_factory() as db:
        # Zero amount
        res = await reserve_revenue_allocation(db, user_id=None, source_event_id="e", square_payment_id="p", payment_tier="t", gross_amount_cents=0)
        assert res is None
        
        # Missing event_id
        res = await reserve_revenue_allocation(db, user_id=None, source_event_id="", square_payment_id="p", payment_tier="t", gross_amount_cents=100)
        assert res is None
