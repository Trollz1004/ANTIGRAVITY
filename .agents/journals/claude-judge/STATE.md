...
### 2026-09-03 — Chief of Staff seats set up properly (Joshua disabled all heartbeats until this was done)

- found: AIS and DRE already had a `Chief of staff` agent (`freebuff_local`) — both in **error** ("freebuff session did not complete the wake within 900s") because their adapterConfig held only instruction paths (no mode/model/effort/timeout like the working HERMES seat) and pointed at nothing. ANT had no seat. YOU is archived (Joshua) — its `OpenCode Engineer` heartbeat refused to disable with a schema validation error; left alone on his word.
- did: PATCHed AIS (`23f8a171…`) and DRE (`ebad4887…`) seats — title "Chief of Staff", HERMES's adapter shape (`timeoutSec` 20000, the 56-skill `paperclipSkillSync` set, managed bundle), `instructionsFilePath` → `ops/paperclip-chief-of-staff/CHIEF-OF-STAFF-{AIS,DRE}.md`. Created ANT seat `65bfc926-5ddc-4022-81c3-19ab359812a9` the same way → `CHIEF-OF-STAFF-ANT.md`; heartbeat created **disabled** so Joshua turns it on. All three seats now load their prompt from the repo file at wake — no paste needed; edits to the md files take effect next wake.
- verified: PATCH 200 ×2, POST 201, listing shows all three seats with the right instruction files. Status on AIS/DRE still reads `error` until their next wake clears it.
- next (Joshua): flip the three Chief of Staff timer heartbeats on when ready (Settings → Heartbeats); leave the 15 SEO agents at 0s (they are on-demand `process` agents run by the schedule); the CoS prompts tell each seat to create the roles from `roles/<company>/`.

## 2026-09-06 05:00–06:00 EDT — Fable model, OmniRoute 3.8.50, healer trap (d92fb0de)

- **joshlcoleman/Fable** built from the Llama 3.2 3B base (tool-capable), SYSTEM = House voice + business-only rule + adults-only 18+ venue policy; tested (pillars + 18+ forum post: 0 guard hits, 0 refusals; the 3B base still invents small perks, so [check] tags matter); pushed to Joshua's **private** ollama.com listing. Ruled the mandatory Date App drafting voice; `dateapp-marketing` retired from public copy. Modelfile + README `ops/fable-model/`. `drift fable` opens it. Sentry Ollama row + House Ollama stage identity = the model in `/api/tags`.
- **OmniRoute 3.8.49 → 3.8.50.** Trap chain, all now recorded at the OmniRoute stage of `FABLES-HOUSE.ps1`: npm EBUSY on `dist` while the gateway runs; two healers (watchdog + `omniroute-keepalive.ps1`) relaunch it in seconds; `--allow-scripts` needed or postinstalls are skipped silently; first boot after upgrade ~90 s. **3.8.50 answers `/v1/models` 401 without the API key** — every unauthenticated probe read DOWN, the House heal-spawned a gateway + dashboard tab a minute (Joshua: "getting spammed"). Fix: Sentry `authEnv`, House `Test-Http` bearer env, keepalive on `/api/health`. Dashboard login switched off: `key_value settings.requireLogin=false` (backup `db_pre-requirelogin-false-*.sqlite`), `/dashboard` 200 unauth. VERIFIED: authed catalog 200 with identity, one gateway, watchdog passes the stage, Sentry 19/33 with omniroute + ollama UP.
- Kill-own-shell lesson: a Stop-Process filter whose regex text appears in its own command line kills the calling shell (exit 255). Use `[.]` and exclude `$PID`/parent.
- Shriners: Ryan replied on the real thread, back Tue 2026-09-08 (memory `revenue-split-unvalidated` updated).
- Still Joshua's: paste README onto the ollama.com page (or say the word and I do it via Chrome), everything in the 09-03 pending list.

## 2026-09-08 11:19–11:35 EDT — avatar through OmniRoute, Pi frontmatter, AI Dev Gallery (9c6aa4f2)

- Joshua ran Pi (`ollama launch pi`): 7 SKILL.md files failed YAML on an unquoted `lane: ...` colon in `description`; quoted them (5 tracked + 2 gitignored `--hash` clones). The `--hash` clone collisions Pi lists are the Paperclip-materialized copies CLAUDE.md already documents; Pi picks the un-hashed one, no action.
- He is "over Paperclip, waste of tokens", working in a Hermes session; heartbeats stay off. No Paperclip action taken.
- Avatar: he first asked for a ComfyUI-type `workflow_api.json`, then said **OmniRoute models instead**. Landed both: `fable omni image-gen` (POST /images/generations, VERIFIED with `antigravity/gemini-3.1-flash-image`, 200 in ~10 s; every other image-looking id is 400 "Invalid image model" — the route keeps its own provider list; no video model in the catalog, /videos still UNVERIFIED) and `fable workflow avatar` (chat drafts → image-gen renders → packet line). Ran end to end: `ops/avatar/out/fable-avatar.png`, illustrated original character, no text. ComfyUI kit at `ops/avatar/comfyui/` (SDXL still, Wan 2.2 TI2V 5B i2v via the GGUF already in the HF cache, stdlib runner, AMD/DirectML install notes) kept as the offline fallback.
- Box facts: no ComfyUI, port 8188 closed; GPU RX 6800 on Windows 10 (DirectML/ZLUDA, not CUDA). **AI Dev Gallery** 0.6.0 (Store) cache `~/.cache/aigallery` holds SD 1.4 ONNX, Whisper Tiny, MiniLM; its `foundry.modelinfo.json` is a 41-entry CPU-only Foundry Local catalog, nothing downloaded, Foundry Local not installed. HF cache has complete Wan 2.2 TI2V 5B (Diffusers + Q4 GGUF) and Z-Image-Turbo bnb-4bit.

## 2026-09-10 02:14–05:30 EDT — "no more paperclip": MC5 is Mission Control, AIRI dashboard on the LAN, House restarts stale services (b49ca141 + follow-up)

- **Ruling recorded (CLAUDE.md):** Mission Control = `mission-control-v5` on `http://192.168.0.8:3151/` (LAN bind, required House stage, real data only). **Paperclip PARKED**: stopped, `ANT-Paperclip` task disabled, House stage report-only, CEO bridge + lanes stages gone, Sentry rows retired. History kept.
- **MC5 fixed, gate green** (typecheck x2, build, 31+3 tests, role wall): 3 dead files deleted; CORS allows private-network origins; `POST /api/launch/claude` + CLAUDE CLI header button; swarm honors agentIds/mode/executor; synthetic subagents never green; frontend probed; human tools probed; Obsidian card reads the vault live (71 notes / 7 wikilinks / 62 orphans); Hermes (`platform:hermes-agent` on :8642) and OpenClaw (`status:live`) probes UP; OmniRoute probe authenticated (server/.env, gitignored); paperclip-9020 fixture removed from the prod queue. Remaining NOT CONFIGURED rows (OmniRoute MCP, onemin-shim) are honest.
- **AIRI dashboard** (`ops/dashboard-airi`, first time tracked) served by `server.mjs` on `0.0.0.0:9150`: OmniRoute proxied with the key server-side (3161 models), 98 live agents from the skills tree, the Obsidian vault as the knowledge graph (click = read + `obsidian://` link), Sentry snapshot, 4 real avatar renders + honest empty VRM viewer, Mission Control embedded from the page host (verified over 192.168.0.8), Claude CLI tab. 16 vitest tests. The old tab bug (missing `#`) is gone.
- **House hardening (learned the hard way):** the logon task runs the House elevated; services it starts cannot be restarted by a non-elevated shell (Sentry kept showing Paperclip rows for an hour). Now: one House at a time (guard), staleness rule (`startedAt` older than code = restart; Sentry/AIRI expose `startedAt`, MC5 `/api/identity`), MC5 must be `0.0.0.0` + fresh, AIRI stage, bring-up spawns the elevated watchdog itself, Startup-folder launcher retired. **Never edit the House with a blind `index()` of a repeated string** — I clobbered the script once (matched the `if (-not $Watchdog)` inside `Log`) and rebuilt it from HEAD + the patch scripts.
- `drift mc` / `drift avatar` added; Sentry gains the AIRI row and `startedAt`.
- Claude launcher: `cmd /c start "Claude (official CLI)" /D … drift.cmd bare` with `windowsVerbatimArguments` (Node re-quotes the title otherwise and `start` eats it). A window titled "Claude (official CLI)" is open on SABRETOOTH as proof.
- Joshua's: verify `http://192.168.0.8:3151/` and `:9150/` from the Alienware (if blocked, an admin firewall rule for node.exe/python.exe is the fix); the Alienware Aurora R12 has the AMD 16 GB card — AMD Adrenalin (`winget install AMD.AdrenalinSoftware`?) not NVIDIA drivers; TTS credential for the gateway; ComfyUI install; a `.vrm` if the 3D avatar is wanted.

## 2026-09-10 06:30–06:45 EDT — launcher form, and the duplicate-session incident (follow-up to b49ca141)

- **Duplicate of this session:** testing `POST /api/launch/claude` from this judge session opened `drift bare` = `claude --continue`, which resumes the LATEST conversation in `C:\ANTIGRAVITY` — i.e. this one. A second instance of the same session then worked the same goal in parallel and made b49ca141 / 2a56a4c2 / 2df62fd6 (all correct, kept). Rule from now on: **never test-launch the Claude launcher from a Claude session**; verify the spawn mechanism with an inert command instead. The launcher itself is VERIFIED by that very incident (the elevated AIRI server spawned it).
- Launchers now use `Start-Process cmd.exe /k "<drift>" bare` (window stays open after the CLI exits so errors are visible); `cmd /c start …` also worked from the elevated servers but its window was invisible to a non-elevated process list, which is what made it look broken.
- MC5 `/api/services` now honest: Hermes via its API server `:8642/health` (`platform: hermes-agent`), OpenClaw `/health` (`status: live`), OmniRoute UP with the key in `server/.env` (copied from the repo `.env`, never printed). Remaining NOT CONFIGURED rows (OmniRoute MCP status, onemin-shim) are true.
- Browser-verified over the LAN host (`http://192.168.0.8:9150/`): dashboard stats live (29/31 services, 98 skills, 71 vault notes, Obsidian UP, 4 renders, 3161 OmniRoute models), Mission Control embedded with its API identified, Knowledge Graph shows the real vault (hubs: 00 HOME, 59s-pipeline, SKILL, DREAM ONLINE, miyo…), Avatar tab shows the 4 real renders and an honestly empty VRM viewer. Console errors seen were transient `:3151` resets while the watchdog restarted MC5, plus a favicon 404 now fixed.
- Joshua's clicks: from the Alienware open `http://192.168.0.8:3151/` and `:9150/`; if either times out, an admin firewall rule for `node.exe` (or ports 3151/9150) on SABRETOOTH is the fix. Alienware GPU: it has the AMD 16 GB card, so the NVIDIA App/driver installs from the phone paste do not apply — `winget install AMD.AdrenalinSoftware` (or the Aurora R12 support page) is the driver route.
