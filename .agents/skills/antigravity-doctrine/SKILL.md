...
4. **10% per-bucket mission reserve (hard cap)** — Every legally distinct revenue stream reserves max 10% (the IRS charitable deduction cap). Per-bucket compounding + per-surface stacking allowed. The 10% reserve is the founder’s taxable income earmarked for the mission; the remaining 90% is the LLC’s taxable operating income, which Josh decides how to allocate.
