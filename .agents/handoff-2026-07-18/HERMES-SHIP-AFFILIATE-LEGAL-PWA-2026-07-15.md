...
1. **Report:** `POST /api/report {targetType:"profile"|"message"|"board_post", targetId, reason, details?}` — validated (enum + length caps), parameterized/no string-built queries, auth-required, rate-limited 30/min/IP. **Emit into the EXISTING SUPPORTCLAW ticket pipeline** (the same path producing Telegram tickets today — reuse it, do not build a parallel sink) AND append to `logs/moderation/reports.jsonl` as local evidence (IDs only, no message bodies). Categories include `child_safety` — flagged `priority:true` and the priority flag must survive into the ticket. **PII rule for launch:** once real users exist, tickets pushed to Telegram carry user IDs only, never emails/names — verify the ticket template and fix if it embeds reporter email.
