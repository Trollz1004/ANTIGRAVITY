...
2. Prior `ghp_*` classic PATs — **all three discovered on-disk PATs verified already dead** this session (`ghp_0ywuMd...`, `ghp_sFCyy5...`, `ghp_scDB4J...` — all return HTTP 401 to `GET /user`). The 9020 doctrine memory `project_repo_consolidation_2026-05-11.md` mentioned `ghp_0ywuMdz...` specifically as pending revocation — that one is now confirmed dead.
3. `github_pat_11BW...` (AidoesitAll fine-grained PAT) — **full token string not found on disk** in OneDrive, AppData, or the repo, so the gist-burn recipe can't be applied (it needs the full token to post). AidoesitAll account is still live on GitHub (not deleted). Two paths to close this out: (a) Josh signs into AidoesitAll once and revokes from the fine-grained-PAT settings page, or (b) delete the AidoesitAll account entirely, which nukes all its tokens server-side. Either is a one-time UI step.
