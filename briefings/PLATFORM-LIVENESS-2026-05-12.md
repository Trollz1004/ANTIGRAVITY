...
Per-service `live` / `dead` / `unknown` status. Credentials read from vault; values never recorded.

| # | Service | Env Key(s) | Status | HTTP | Notes |
|---|---------|------------|--------|------|-------|
| 1 | Stripe | STRIPE_SECRET_KEY | **live** | 200 | account.id=acct_…SQoI (from recovery vault) |
| 2 | Cloudflare | CLOUDFLARE_API_TOKEN | **dead** | 401 | "Invalid API Token" — token in vault is stale/rotated |
| 3 | Square | SQUARE_ACCESS_TOKEN | **live** | 200 | 2 locations returned, id=…41TH |
| 4 | eBay | EBAY_AUTH_TOKEN | **dead** | 401 | errorId=1001 "Invalid access token" — IAF token expired; needs OAuth2 refresh |
| 5 | Anthropic | ANTHROPIC_API_KEY | **live** | 200 | 9 models listed (recovery vault key) |
| 6 | OpenAI | OPENAI_API_KEY | **dead** | 401 | Primary key invalid; OPENAI_ALT_KEY in recovery vault is live (200, 119 models) — canonical key needs updating |
...
| 14 | Twitter/X | TWITTER_BEARER_TOKEN | **unknown** | 403 | Bearer token format valid; 403 is tier restriction (App-only auth blocked on free API for /users/me) — token not definitively dead, but cannot confirm live without paid tier endpoint |
...
| 16 | Replicate | REPLICATE_API_KEY | **dead** | 401 | "Unauthenticated — You did not pass a valid authentication token" |
| 17 | HuggingFace | HUGGINGFACE_API_KEY | **dead** | 401 | Token named "NSFW Platform" is explicitly expired per API response |
| 18 | Meta/FB | META_ACCESS_TOKEN | **unknown** | N/A | META_ACCESS_TOKEN is blank in vault — never set |
| 19 | xAI/Grok | XAI_API_KEY | **dead** | 400 | Recovery vault XAI_API_KEY is invalid (400); GROK_API_KEY from MASTER vault is live (200, 16 models) — wrong key promoted to canonical |
| 20 | Stability | STABILITY_API_KEY | **live** | 200 | Account confirmed, id=user-…iEe3 (requires browser UA to bypass Cloudflare WAF) |

---

## Summary
- **Live: 9** (Stripe, Square, Anthropic, Gemini, SendGrid, Plaid, Telegram, Stability, + xAI via MASTER GROK_API_KEY)
- **Dead: 5** (action needed — rotation list below)
- **Unknown / no key: 6**
...
|---------|---------|-------|
| Cloudflare | CLOUDFLARE_API_TOKEN / CF_API_TOKEN | Token rotated or expired; generate new token at dash.cloudflare.com → My Profile → API Tokens |
| eBay | EBAY_AUTH_TOKEN | Legacy IAF token expired; must re-authenticate via eBay OAuth2 consent flow and store new token |
...
| Replicate | REPLICATE_API_KEY | Token invalid; regenerate at replicate.com/account/api-tokens |
| HuggingFace | HUGGINGFACE_API_KEY | "NSFW Platform" token explicitly expired; regenerate at huggingface.co/settings/tokens |

**Additionally — wrong key in canonical position:**
- `XAI_API_KEY` in recovery vault is dead (400). `GROK_API_KEY` in MASTER vault IS live (16 models). Promote GROK_API_KEY value to XAI_API_KEY in both vaults for consistency.
...
1. **Twitter/X (14)**: Bearer token returns 403 "App-only auth forbidden" for `/users/me` — this is a Twitter free-tier API restriction, not an authentication failure. The token may be valid for other endpoints (search requires Project-attached app). Recommend testing with `/2/tweets/search/recent` on a Project-attached app.
