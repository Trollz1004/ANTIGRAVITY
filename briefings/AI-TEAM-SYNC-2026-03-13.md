...
Current baseline: repo tip at read time
...
## OpenClaw / Grok Reality

- Sabretooth is the only verified OpenClaw orchestrator runtime
- Local gateway: `ws://127.0.0.1:18789`
- Use `openclaw status` as runtime truth
- Sabretooth is the only Telegram-owning OpenClaw instance
- Grok is now available as an OpenClaw-backed local agent on Sabretooth
...
- routes Gemini, Claude, Grok, and local worker tasks
...
- no remote Telegram pollers besides Sabretooth
