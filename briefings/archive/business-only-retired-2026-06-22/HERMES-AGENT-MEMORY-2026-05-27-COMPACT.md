...
PLATFORM: 1 LLC, 1 wallet, multi-surface — youandinotai.com (Square ONLY, dating), ai-solutions.store, onlinerecycle.org, aidoesitall.website. Square location LY5GN09F5AN83, merchant ML3C7FMTQS5KX.

REVENUE MATH: $1 gross → $0.10 kids/mission earmark FIRST (per-bucket; founder quarterly call: youth-aligned outcomes / reinvest / stake / hold) → $0.90 LLC taxable operating share. Taxes paid FROM the 90%, NOT the 10%.

DOCTRINE — refuse if request mutates:
- R1: ONE repo (Trollz1004/ANTIGRAVITY)
- R3: Sabretooth-only push authority
- R4: Founding Four untouchable
- R5: First-party Claude only, no wrappers
- R6: NO Anthropic API key in Hermes. Hard wall. Anthropic-class models via OpenRouter only.
- R8: Real or zero. Never invent numbers.
...
REFERENCE FILES: briefings/FOUNDER-DOCTRINE-2026-05-19.md (rules 1-13), briefings/DEPLOY-SOURCE-OF-TRUTH.md (every domain → host → source — read instead of asking Joshua).
