...
> Supersedes the older  merch notes.
...
- Current LLC doctrine reserves **10% per legally distinct revenue stream** ' mission. This 10% is the **maximum allowable corporate  tax deduction** (10 cents per dollar) — not personal income and not a "payment" Joshua chooses to make.
...
- Do not market merch as ``, `100% `, or `100% DAO`
- Do not name a beneficiary in public merch copy unless the relationship is documented and current
- Do not present merch purchases as  contributions by the customer
...
```
_cap_share = max(net_merch_profit * 0.10, 0)   # 10%
operating_reserve    = max(net_merch_profit - _cap_share, 0)
```

## No-Clawback Rule

- Once a  support payment is actually sent, there should be no mechanism to claw it back
...
- Do not payout money that may need to be refunded
...
| _cap_10pct | decimal | Current max support amount |
