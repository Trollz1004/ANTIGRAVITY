...
- Delta since previous:
  ...
  - Revenue model reframed 2026-06-01: 10% per-bucket is the IRS LLC -deduction cap (max 10c/$ corporate deduction), NOT Joshua's personal income. ``, Gospelpayment.sol, Router100.sol, "100% " framing all formally dead.
...
- Canonical-7 ban on customer surfaces: CHECK FAILED on the following paths —
  - `apps/youandinotai-frontend/components/Section.tsx` (3 hits)
...
- Customer-facing canonical-7 violations live in 7 files (one Next.js component named `Section.tsx`, and the `_deploy/dao-launch`, `_deploy/dao-transparency`, `_deploy/onlinerecycle/*` bundles). Surgical substitution + re-deploy required before any new customer-facing push.
...
- The 7 canonical-7 hits in `_deploy/` and `apps/youandinotai-frontend/components/Section.tsx` — clear to schedule a surgical-substitution PR (per the HTML bundle drop routine: `payment→support, →mission, →mission-aligned, payout→contractual revenue transfer` agent-internal-only never customer-facing), or do you want to handle this manually?
