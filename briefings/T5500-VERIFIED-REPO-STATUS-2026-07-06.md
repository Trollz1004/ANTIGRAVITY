...
Keep platform and customer-facing language commercial, product-first, and revenue-oriented. Avoid legacy guardrail phrasing in new repo work.
...
## 17. Hermes workspace/dashboard consolidation verified on T5500

Current live Hermes surface on T5500 is stable:

```text
Workspace: https://hermes-workspace.youandinotai.com  -> 127.0.0.1:3000
Dashboard: https://hermes-dashboard.youandinotai.com  -> 127.0.0.1:9119
API:       http://127.0.0.1:8642
```

Runtime launcher fix applied on this workstation:

- `C:\Users\joshl\AppData\Local\hermes\scripts\start-hermes-workspace-3000.ps1`
- The launcher now loads `C:\Users\joshl\hermes-workspace\.env` in PowerShell and starts `node.exe server-entry.js` directly.
- This replaced the brittle bash shell-redirection path that was preventing `3000` from coming up reliably under the Windows task.

Verification performed after the fix:

```text
3000 listening on 0.0.0.0 (PID 9752)
Hermes_Workspace_3000 + Hermes_Workspace_3000_SYSTEM = Running
browser-like GET to hermes-workspace.youandinotai.com => 200
browser-like GET to hermes-dashboard.youandinotai.com => 200
```

Cloudflared route source of truth:

- `C:\Users\joshl\.cloudflared-hermes\config.yml`
- `hermes-workspace.youandinotai.com` -> `http://127.0.0.1:3000`
- `hermes-dashboard.youandinotai.com` -> `http://127.0.0.1:9119`

Post-reboot correction on 2026-07-07:

- `Hermes_Workspace_3000_SYSTEM` is disabled because the SYSTEM-owned Node workspace process can hold `:3000` while HTTP is unhealthy.
- `Hermes_Workspace_3000` remains the canonical workspace startup task at user logon.
- `HermesWorkspaceWatchdog.vbs` is installed in the user Startup folder as a second user-logon launcher for the workspace watchdog.
- `C:\Users\joshl\AppData\Local\hermes\scripts\start-hermes-workspace-3000.ps1` now checks HTTP health, not only the TCP listener, before accepting `:3000` as healthy.
- `OllamaWatchdog.vbs` is installed in the user Startup folder and launches `C:\ANTIGRAVITY\scripts\watchdog\ollama-watchdog.ps1` hidden at logon.
