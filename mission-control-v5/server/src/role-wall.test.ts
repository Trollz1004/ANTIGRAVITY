...
// fileURLToPath keeps the checker path valid on Windows (URL.pathname yields /C:/...).
const checker = fileURLToPath(new URL('../scripts/role-wall-check.mjs', import.meta.url));
...
function runChecker(root: string): string {
  return execFileSync(process.execPath, [checker, root], { encoding: 'utf8' });
}
