...
describe('JoshuaCLAW Governance - VOTERS', () => {
  ...
  it('all 6 AI platforms are represented', () => {
    ...
    expect(aiSlugs).toEqual(expect.arrayContaining(['manus', 'claude', 'gemini', 'perplexity', 'grok', 'codex']));
  });
...
describe('JoshuaCLAW Governance - TIER_CONFIG', () => {
  ...
  it('critical tier covers revenue, contracts, and Iron Wall', () => {
    expect(TIER_CONFIG.critical.categories).toEqual(expect.arrayContaining(['', 'contract-deploy', 'iron-wall']));
  });

  it('operational tier covers bug fixes, UI, and marketing', () => {
    expect(TIER_CONFIG.operational.categories).toEqual(expect.arrayContaining(['bug-fix', 'ui-update', 'marketing']));
  });
