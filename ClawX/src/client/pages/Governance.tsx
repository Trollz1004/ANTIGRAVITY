...
import {
  Shield, Bot, Brain, Sparkles, Search, Zap, Server, Code2,
  User, Plus, CheckCircle2, XCircle, Clock, AlertTriangle,
  Vote, Lock, Unlock, TrendingUp,
} from "lucide-react";

const voterIcons: Record<string, React.ElementType> = {
  joshua: User, manus: Bot, claude: Brain,
  gemini: Sparkles, perplexity: Search, grok: Zap, codex: Code2,
};

const voterColors: Record<string, string> = {
  joshua: "#f59e0b", manus: "#6366f1", claude: "#d97706",
  gemini: "#4285f4", perplexity: "#22c55e", grok: "#ef4444", codex: "#8b5cf6",
};
...
function VoterCard({
  name, role, type, position, icon: Icon, color, isHuman,
}: {
  name: string; role: string; type: string; position: number;
  icon: React.ElementType; color: string; isHuman: boolean;
}) {
  return (
    <div className="relative flex flex-col items-center gap-2 p-3 rounded-xl border border-border/50 bg-secondary/20 hover:bg-secondary/40 transition-colors">
      <div className="absolute top-1.5 right-1.5 text-[10px] font-mono text-muted-foreground/50">
        #{position}
      </div>
      <div
...
function ProposalCard({
  proposal, isSelected, onSelect, onVoteSuccess,
}: {
  proposal: any; isSelected: boolean; onSelect: () => void; onVoteSuccess: () => void;
}) {
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <Badge variant="outline" className="text-[10px] px-1.5 gap-1" style={{ borderColor: tier.color, color: tier.color }}>
            ...
            <div className="relative h-14 w-14">
              <svg className="h-14 w-14 -rotate-90" viewBox="0 0 56 56">
                ...
                <circle
                  ...
                  stroke={proposal.status === "approved" ? "#22c55e" : proposal.status === "rejected" ? "#ef4444" : "#6366f1"}
                  strokeWidth="4"
...
function ProposalVoteDetail({
  proposalId, status, onVoteSuccess,
}: {
  proposalId: number; status: string; onVoteSuccess: () => void;
}) {
    ...
    <div className="mt-4 pt-4 border-t border-border/50" onClick={(e) => e.stopPropagation()}>
        ...
        {voters?.map((voter) => {
          ...
          return (
            <div key={voter.slug} className="flex flex-col items-center gap-1.5 p-2 rounded-lg bg-secondary/20">
              ...
              {hasVoted ? (
                <span className={`text-[9px] font-bold ${existingVote?.vote === "approve" ? "text-green-500" : "text-red-500"}`}>
      ...
      {votes && votes.length > 0 && (
        <div className="mt-3 space-y-1.5">
          ...
          {votes.filter((v) => v.reasoning).map((v) => {
...
function CreateProposalForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const categories = tier === "critical"
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Update revenue split to 70/20/10" />
      </div>
      <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label>Tier</Label>
          <Select value={tier} onValueChange={(v) => { setTier(v as any); setCategory(""); }}>
