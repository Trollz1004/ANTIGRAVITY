...
export async function getUserApiKeyMeta(userId: number): Promise<
  Array<{ providerSlug: string; keyHint: string | null; label: string | null; isActive: number }>
> {
  const db = await getDb();
  if (!db) return [];
  const rows = await db
...
export async function getUserApiKeys(
  userId: number
): Promise<Partial<Record<AiProviderSlug, string>>> {
  const db = await getDb();
  if (!db) return {};
  const rows = await db
    .select({ providerSlug: userApiKeys.providerSlug, encryptedKey: userApiKeys.encryptedKey, isActive: userApiKeys.isActive })
    .from(userApiKeys)
    .where(eq(userApiKeys.userId, userId));
