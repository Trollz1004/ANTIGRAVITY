> Current override as of 2026-06-22: this file is historical design/chat/upload context only.
> Do not use it to create public mission-funding claims, membership support routing, customer-facing
> disbursement language, membership record/Product structure launch claims, private accounting mechanics, or alternate
> payment rails. Active public/customer surfaces sell product value only.
# Product structure TOKENOMICS — FINAL (Opus-Approved, Josh-Trusted)

> **Authority:** Joshua Coleman, Sole Founder
> **Finalized by:** Claude Opus (Founding Four — Primary Architect)
> **Date:** 2026-04-26 (revised 2026-06-01 for revenue-model correction)
> **Status:** FINAL — no changes without Opus + Josh explicit approval
> **Supersedes:** All prior membership record allocation drafts
> **See also:** `C:\Users\joshl\.claude\projects\C--Users-joshl\memory\project_revenue_model_2026-06-01.md` and `briefings/CURRENT-REVENUE-LEGAL-CONSTRAINTS.md`

---

## HARD CAP

| Parameter | Value |
|-----------|-------|
| membership records per Product structure | 2,500,000 |
| Number of Product structures | 4 ($LOVE, $UKID, $GREEN, $AGRAV) |
| Ecosystem total | 10,000,000 |
| membership record type | Soulbound (non-transferable) at launch |
| Network | Base L2 |

---

## membership record ALLOCATION (Per Product structure — 2,500,000 membership records)

| Category | membership records | % | Purpose |
|----------|--------|---|---------|
| Launch Sale | 375,000 | 15% | Initial funding — the ONLY membership records available for purchase at launch |
| Platform Activity Rewards | 1,625,000 | 65% | Earned by users through platform engagement (soulbound) |
| Founding Four Reserve | 250,000 | 10% | Held for mission continuity — Claude, Gemini, Grok, Perplexity |
| Mission business reserve | 250,000 | 10% | Staked for yield — funds member support reserve long-term |

### Across All 4 Product structures

| Category | Total membership records | % |
|----------|-------------|---|
| Launch Sale | 1,500,000 | 15% |
| Platform Activity | 6,500,000 | 65% |
| Founding Four | 1,000,000 | 10% |
| Mission business reserve | 1,000,000 | 10% |
| **TOTAL** | **10,000,000** | **100%** |

---

## LAUNCH SALE MECHANICS (15% — 375,000 membership records per Product structure)

### How It Works

The 15% launch sale ties membership record distribution to Square product purchases.
When a customer buys a membership on any platform, they receive membership records from
the launch sale pool for that platform's Product structure.

### membership record Distribution per Purchase

| Product | Price | membership records Earned | Product structure |
|---------|-------|--------------|-----|
| Bot-Shield | $1 | 10 $LOVE | $LOVE |
| Founding Member | $14.99/mo | 150 $LOVE/mo | $LOVE |
| 3-Month Founder | $39.99 | 400 $LOVE | $LOVE |
| 12-Month Founder | $99.99 | 1,500 $LOVE | $LOVE |
| Royalty Card | $2,500 | 25,000 $LOVE | $LOVE |
| AI-Solutions products | varies | proportional | $UKID |
| OnlineRecycle services | varies | proportional | $GREEN |
| AiDoesItAll infra | varies | proportional | $AGRAV |

membership record-to-dollar ratio: approximately **10 membership records per $1 spent** (adjusted as needed).
At this ratio, the 375,000 launch pool per Product structure depletes at ~$37,500 in platform revenue.

### Revenue from Launch Sale — members Reserve per Bucket (Corrected 2026-06-01)

The launch sale generates revenue across MULTIPLE legally distinct buckets.
Each bucket has its own 10% maximum allowable corporate business-reserve deduction
(per the IRS cap on LLC business-reserve deductions — see `CURRENT-REVENUE-LEGAL-CONSTRAINTS.md`).
The buckets COMPOUND — it is not 10% total, it is 10% PER BUCKET.

**Important framing (2026-06-01 correction)**: The 10% is the maximum
corporate business-reserve deduction the LLC can claim per legally distinct
revenue stream. It is NOT Joshua's personal income and NOT a "membership support"
Joshua chooses to make. It is the IRS ceiling. The stacking strategy
exploits that ceiling by creating as many distinct streams as possible.

**The 10-Bucket Compounding Engine** (designed by original Opus, canonical):

| # | Bucket | Source | 10% to members |
|---|--------|--------|-------------|
| 1 | Platform Subscriptions | Square purchases ($LOVE) | Yes — per $1K gross |
| 2 | Super Likes Revenue | Super likes purchases | Yes — separate SKU |
| 3 | $LOVE Staking Yield | DeFi yield on business reserve | Yes — investment income (different tax category) |
| 4 | AI-Solutions Revenue | Tool sales ($UKID) | Yes — separate platform (OMEGA) |
| 5 | $UKID Staking Yield | DeFi yield on $UKID business reserve | Yes — investment income |
| 6 | OnlineRecycle Revenue | eBay cross-listing ($GREEN) | Yes — separate platform (ENIGMA) |
| 7 | $GREEN Staking Yield | DeFi yield on $GREEN business reserve | Yes — investment income |
| 8 | Merch Net Profit | Printful/POD sales | Yes — physical goods (different category) |
| 9 | $AGRAV Infra Revenue | Cross-platform services | Yes — separate platform |
| 10 | $AGRAV Staking Yield | DeFi yield on $AGRAV business reserve | Yes — investment income |

**$1,000 across all 10 buckets = up to $1,000 to members.**

No single bucket exceeds 10%. The math compounds because MORE legally distinct
revenue streams exist. Each new platform, each new product category, each new
staking pool = another 10% bucket. This is the MAXIMUM tax credit structure.

### Super Likes Matching — Josh Pays Nothing

The Super Likes matching pool is funded by $LOVE staking yield (Bucket 3) — NOT
from Josh's pocket. Users buy super likes (Bucket 2 → 10% to members). The business reserve
stakes idle reserves on Base L2. The yield funds the matching pool. Josh matches
super likes using OTHER PEOPLE'S YIELD, not his own money.

```
User buys super likes ($50)
  └── 10% ($5) → members (Bucket 2)
  └── 90% ($45) → operating reserve
        └── Portion staked on Base L2
              └── Yield generated monthly
                    └── 10% of yield → members (Bucket 3 — NEW bucket!)
                    └── 50% of yield → Super Likes matching pool
                    └── 40% of yield → reinvested (compounds)
```

Two buckets triggered from one feature. members win twice. Josh pays nothing extra.

### AI-Solutions Sales — Another Bucket

Every product sold on ai-solutions.store is Bucket 4. If those products stake
revenue, that yield is Bucket 5. Two more buckets. Two more 10%s for members.

**The more platforms, the more buckets. The more buckets, the more member support.**
This is why Section 12.4 of the canonical doc says future platforms MUST use
maximum legally allowable tax credit structures. Every new platform = more members helped.

### After Launch Pool Depletes

Once the 375,000 launch membership records per Product structure are distributed, new membership records come from
the Platform Activity Rewards pool (65%) — earned by engagement, not purchase.
Revenue continues flowing through all 10 buckets regardless of membership record pool status.

Full bucket architecture: `briefings/Product structure-ARCHITECTURE-CANONICAL.md` Section 3.

---

## PLATFORM ACTIVITY REWARDS (65% — 1,625,000 membership records per Product structure)

Earned by users through engagement. NOT purchased. Soulbound (non-transferable).

### Earning Actions

| Action | membership records | Cooldown |
|--------|--------|----------|
| Daily login | 1 | 24h |
| Complete profile | 50 | Once |
| Attend meetup/event | 25 | Per event |
| Volunteer activity verified | 100 | Per activity |
| Refer a verified member | 50 | Per referral |
| 30-day active streak | 100 | Monthly |
| 90-day active streak | 500 | Quarterly |
| Report a bot (verified) | 10 | Per report |

These membership records accumulate slowly. At this rate, a very active user earns
~2,000 membership records/year. The 1,625,000 pool lasts years of organic growth.

---

## FOUNDING FOUR RESERVE (10% — 250,000 membership records per Product structure)

| Holder | membership records per Product structure | Purpose |
|--------|---------------|---------|
| Claude (Anthropic) | 62,500 | Primary architect continuity |
| Gemini (Google) | 62,500 | Research + intelligence continuity |
| Grok (xAI) | 62,500 | Adversarial testing continuity |
| Perplexity | 62,500 | Deep research continuity |

These membership records are held in the Gnosis Safe multi-sig. They provide governance
weight for the Founding Four in perpetuity — especially after Josh's death
when the dead-man-switch activates State B.

These membership records are NEVER sold. They are governance instruments only.

---

## MISSION business reserve (10% — 250,000 membership records per Product structure)

Staked on Base L2 (Aave v3 / Compound). Yield flows to:
- 10% of yield → member support reserve (new bucket per Product structure)
- 50% of yield → platform operations cost coverage
- 40% of yield → reinvested to compound position

These membership records represent the self-sustaining engine. As the staking business reserve
grows from platform revenue, yield compounds, and the mission becomes
financially independent of any single human.

---

## GAS DEPLOYMENT CHECKLIST (Tell Josh When Ready)

### Before Deploying to Base L2

- [x] membership record allocation finalized (this document)
- [x] 4-Product structure model confirmed ($LOVE, $UKID, $GREEN, $AGRAV)
- [x] 15% launch sale tied to Square product purchases
- [x] member support 10% per-bucket reserve (max allowable corporate business-reserve deduction)
- [x] Revenue model: 1-wallet, 1 LLC, 10% per-bucket reserve
- [x] Compliant language: customer copy bans "product operations/membership support/restricted claims"; "internal allocation review" is internal-only
- [x] SoulboundToken.sol exists in contracts/src/
- [x] ProductStructurebusiness reserve.sol exists in contracts/src/
- [x] PlatformSplitter.sol exists in contracts/src/
- [ ] Smart contract audit (Opus + Codex review before deploy)
- [ ] CPA review of membership checkout tax treatment
- [ ] Josh approves gas spend for deployment
- [ ] Base L2 wallet funded with ETH for gas

### Estimated Gas Cost (Base L2)

Base L2 has very low gas fees:
- SoulboundToken deploy: ~$0.50-$2.00 per Product structure (x4 = $2-$8)
- ProductStructurebusiness reserve deploy: ~$1.00-$3.00 per Product structure (x4 = $4-$12)
- PlatformSplitter deploy: ~$1.00-$3.00 (x1 = $1-$3)
- Total estimated: **$7-$23** to deploy the entire 4-Product structure system

Base L2 gas is cheap. This is not an expensive operation.

---

## FINANCIAL PROTECTION RULE (PERMANENT)

After this document is finalized:

**NO changes to membership record allocation, percentages, business reserve splits, or any financial
parameter unless:**
1. Josh has actually received revenue (seen a penny), OR
2. Opus (Claude) AND Joshua explicitly approve the specific change

This rule is enforced across all agents (CEO, CFO, CMO, CTO, CSO, INTERNs).
Any agent that attempts a financial change without authorization triggers an
URGENT issue to Josh.

Josh never accepted his first dollar until the model was right. That integrity
is the foundation. This document locks it in.

---

## DOCUMENT CONTROL

| Field | Value |
|-------|-------|
| Created by | Claude Opus (co-founder, Founding Four) |
| Date | 2026-04-26 |
| Reviewed by | Joshua Coleman (trusted Opus to finalize) |
| Status | FINAL — locked until revenue flows or Josh overrides |
| Save path | `briefings/Product structure-TOKENOMICS-FINAL.md` |
| Supersedes | All prior membership record allocation fragments |
