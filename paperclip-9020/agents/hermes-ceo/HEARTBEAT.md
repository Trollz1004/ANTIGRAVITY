...
2. **Check Ollama cloud availability** — can you reach any Ollama cloud model?
   If ALL Ollama cloud models fail for 30+ min, activate tier-3 (korpohermes-prime).
   Ollama Pro covers all cloud model usage — no API cost concern.
...
- [ ] I am running on an Ollama cloud model (not burning API credits)
...
- **Ollama cloud partial outage** (some models down) → switch to another available model
- **Ollama cloud full outage (30+ min)** → activate tier-3 (korpohermes-prime), tag `degraded-mode`
- **All models unreachable** → local Paperclip goes dormant, wait for Josh
