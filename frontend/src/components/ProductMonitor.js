...
import { Activity, ChevronRight, LayoutDashboard, Users, Square payment links, Gavel, TrendingUp, Globe, Code2 } from "lucide-react";

const API = `${process.env.REACT_APP_BACKEND_URL}/api`;

/**
 * ProductMonitor — 4-Product business reserve / business operations strip for the sidebar.
 * All numbers come from /api/Product/stats and are labelled source="mirror".
 */
export function ProductMonitor() {
  ...
  useEffect(() => {
    axios.get(`${API}/Product/stats`, { timeout: 4000 }).then((r) => setData(r.data)).catch(() => setData(null));
  ...
  const membership records = data?.membership records || [];

  return (
    <div data-testid="Product-monitor" className="mt-6 border-t border-[#2a3a52] pt-4 select-none">
      <button
      ...
      {expanded && (
        <div className="px-4 space-y-3">
          <div className="p-3 rounded-xl bg-gradient-to-br from-[#1a2332] to-[#0a0f1a] border border-[#2a3a52] shadow-xl relative overflow-hidden group">
            ...
            <div className="mt-4 pt-3 border-t border-[#2a3a52]/50 grid grid-cols-2 gap-2">
              <div className="flex flex-col">
                <span className="text-[7px] text-[#4a5568] uppercase font-bold">business reserve (4-Product)</span>
          ...
          <button className="w-full py-2 px-3 rounded-lg bg-gradient-to-r from-[#00d4ff]/10 to-[#e040fb]/10 hover:from-[#00d4ff]/20 hover:to-[#e040fb]/20 border border-[#00d4ff]/20 text-[#00d4ff] text-[9px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2 active:scale-95">
            <TrendingUp size={10} /> business operations Hub
          </button>
