...
export function SystemStatus() {
  ...
  useEffect(() => {
    load();
    const iv = setInterval(() => { if (!document.hidden) load(); }, 15_000);
  ...
  return (
    <div data-testid="system-status" className="bg-[#111827] border-t border-[#2a3a52] p-4 mt-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <button data-testid="system-status-refresh" onClick={load} className="text-[#4a5568] hover:text-[#00d4ff] transition-colors">
