 ...
 * file picker → iframe sandbox="allow-scripts allow-same-origin"
 * Runbooks already ship their own copy-to-clipboard wiring.
 */
    ...
    >
      <div className="bg-[#111827] border-b border-[#2a3a52] px-3 py-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <input
            ref={inputRef}
            data-testid="runbook-file-input"
            type="file"
            accept=".html,.htm"
            onChange={onFile}
            className="hidden"
          />
          <button
            data-testid="runbook-open"
            onClick={() => inputRef.current?.click()}
            className="flex items-center gap-1 text-[8px] tracking-widest uppercase text-[#e040fb] hover:text-[#f070fe] transition-colors"
          >
