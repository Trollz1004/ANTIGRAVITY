...
export function ShareMissionModal({ stats, onClose }) {
  ...
  return (
    <div data-testid="share-modal" className="fixed inset-0 bg-[#0a0f1a]/90 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#1a2332] border border-[#fb923c]/30 rounded-md max-w-3xl w-full overflow-hidden shadow-[0_0_60px_rgba(251,146,60,0.2)]" onClick={(e) => e.stopPropagation()}>
        <div className="bg-[#111827] border-b border-[#2a3a52] px-4 py-2.5 flex items-center justify-between">
          <span className="text-xs font-bold tracking-wide flex items-center gap-2"><Share2 size={12} className="text-[#fb923c]" /> Share Mission Status</span>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex justify-end gap-2">
            <button onClick={onClose} className="text-[10px] tracking-widest uppercase px-3 py-1.5 text-[#6b82a6] hover:text-[#e8f0ff]">close</button>
            <button data-testid="share-download" onClick={download} className="text-[10px] tracking-widest uppercase px-4 py-1.5 rounded bg-[#1a2332] border border-[#2a3a52] hover:border-[#fb923c] flex items-center gap-1.5">
              <Download size={11} /> download png
            </button>
            <button data-testid="share-native" onClick={shareNative} className="text-[10px] tracking-widest uppercase px-4 py-1.5 rounded bg-[#fb923c] text-[#0a0f1a] font-bold hover:bg-[#fbb05a] flex items-center gap-1.5">
