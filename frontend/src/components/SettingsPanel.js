            ...
            {providers.map((p) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[12px] font-bold truncate" style={{ color: p.color }}>{p.label}</div>
                  <div className="text-[9px] mono text-[#6b82a6] truncate">
                    {p.tier === "byok" ? `BYOK · ${p.env}` : p.tier === "local" ? "Local · workstation only" : "Bridged · Emergent universal key"}
                  </div>
                </div>
                <span className={`text-[9px] tracking-widest uppercase px-2 py-0.5 rounded-full border ${
                  p.ready ? "bg-[#00e676]/10 border-[#00e676]/30 text-[#00e676]" : p.tier === "local" ? "bg-[#ffb300]/10 border-[#ffb300]/30 text-[#ffb300]" : "bg-[#ff1744]/10 border-[#ff1744]/30 text-[#ff1744]"
                }`}>
...
function Row({ label, value }) {
  return (
    <div className="flex items-center justify-between gap-4 text-[11px]">
      ...
      <span className="mono text-[#e8f0ff] truncate" title={value}>{value || "—"}</span>
    </div>
  );
}
...
function BroadcastRow({ channel, label, color, status, ping, pingResult, envHint }) {
  ...
  return (
    <div data-testid={`broadcast-row-${channel}`} className="bg-[#0a0f1a] border border-[#2a3a52] rounded p-3 space-y-2">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 min-w-0">
          <Radio size={12} style={{ color }} />
          <span className="text-[12px] font-bold" style={{ color }}>{label}</span>
          {status?.configured ? (
            <span className="flex items-center gap-1 text-[9px] tracking-widest uppercase text-[#00e676]"><CheckCircle2 size={10}/> configured</span>
          ) : (
            <span className="flex items-center gap-1 text-[9px] tracking-widest uppercase text-[#ff1744]"><XCircle size={10}/> not configured</span>
          )}
        </div>
        <button
      ...
      {result?.state === "done" && (
        <div className="mono text-[10px] border-t border-[#2a3a52] pt-2 break-words">
          <span className={result.data.ok ? "text-[#00e676]" : "text-[#ff1744]"}>
            {result.data.ok ? "OK" : "FAIL"}
          </span>{" "}
          <span className="text-[#6b82a6]">{JSON.stringify(result.data).slice(0, 240)}</span>
        </div>
      )}
      {result?.state === "error" && (
        <div className="mono text-[10px] text-[#ff1744]">{result.error}</div>
      )}
    </div>
  );
}
