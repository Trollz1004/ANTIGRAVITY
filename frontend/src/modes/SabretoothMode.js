...
export function SabretoothMode() {
  ...
  const refresh = async () => {
    try {
      ...
      setStatus(r.data); setError(null);
    } catch (e) {
      setStatus(null);
  ...
  const run = async () => {
    ...
    try {
      ...
      setOutput((o) => [{ ts: new Date().toLocaleTimeString(), command: selected, error: e.response?.data?.detail || e.message }, ...o].slice(0, 20));
    ...
    <div data-testid="sabretooth-mode" className="h-full bg-[#0a0f1a] text-[#e8f0ff] overflow-y-auto custom-scrollbar p-6">
      <div className="max-w-5xl mx-auto space-y-5">
        <div className="flex items-center gap-3">
          ...
          <button data-testid="sabretooth-refresh" onClick={refresh} className="ml-auto text-[#6b82a6] hover:text-[#00e676] transition-colors p-2">
            <RefreshCw size={14} />
          </button>
        </div>

        {error && (
          <div data-testid="sabretooth-error" className="bg-[#ff1744]/10 border border-[#ff1744]/30 rounded p-3 mono text-xs text-[#ff1744] flex items-start gap-2">
            <AlertTriangle size={14} className="shrink-0 mt-0.5" />
            <div>
              <div className="font-bold mb-1">Status fetch failed</div>
              <div>{error}</div>
              <div className="mt-1 text-[10px] text-[#6b82a6]">If you see "sign-in required", sign in first; if "admin auth not configured", set ADMIN_PASSWORD in /app/backend/.env.</div>
            </div>
          </div>
        )}
          ...
          <>
            ...
            {configured && !reachable && (
              <div className="bg-[#ff1744]/10 border border-[#ff1744]/30 rounded p-3 mono text-[11px] text-[#ff1744] leading-relaxed">
                ...
                Probe failed: {status.reachability_error || "no route to host"}.
                Wire tailscale or a cloudflare-tunnel from Sabretooth before exec works from this node.
              </div>
            )}
            ...
            <div className="bg-[#1a2332] border border-[#2a3a52] rounded-md">
              <div className="bg-[#111827] border-b border-[#2a3a52] px-3 py-2 flex items-center gap-2">
                ...
                <span className="ml-auto text-[8px] tracking-widest uppercase text-[#6b82a6]">{status.allowed_commands.length}</span>
              </div>
                ...
                <div key={i} className="p-3 space-y-1">
                  <div className="flex items-center gap-2 text-[9px] tracking-widest uppercase">
                    ...
                    {row.exit_code !== undefined && (
                      <span className={row.exit_code === 0 ? "text-[#00e676]" : "text-[#ff1744]"}>exit {row.exit_code}</span>
                    )}
                  </div>
                  {row.error && <pre className="mono text-[11px] text-[#ff1744] whitespace-pre-wrap">{row.error}</pre>}
                  {row.stdout && <pre className="mono text-[11px] text-[#e8f0ff] whitespace-pre-wrap">{row.stdout}</pre>}
                  {row.stderr && <pre className="mono text-[11px] text-[#ffb300] whitespace-pre-wrap">{row.stderr}</pre>}
                </div>
              ))}
            </div>
          </div>
...
function KV({ label, value, mono, tone, icon: Icon }) {
  ...
  return (
    <div className="bg-[#1a2332] border border-[#2a3a52] rounded-md p-3">
      ...
      <div className={mono ? "mono text-sm font-bold truncate" : "text-sm font-bold truncate"} style={{ color }} title={value}>
        {value}
      </div>
    </div>
  );
