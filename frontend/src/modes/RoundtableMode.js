...
export function RoundtableMode() {
  ...
  const send = async () => {
    ...
    await Promise.all(
      Object.entries(active).map(async ([providerId, model]) => {
        try {
          ...
          if (!res.ok) {
            setResponses((prev) => ({
              ...prev,
              [providerId]: { state: 'error', error: body.detail || `HTTP ${res.status}` },
            }));
            return;
          }
          ...
          <button
            ...
            className={`flex items-center gap-1 text-[9px] tracking-widest uppercase px-2 py-1 rounded-full border transition-all ${
              notify
                ...
                : 'bg-[#1a2332] border-[#2a3a52] text-[#6b82a6] hover:border-[#00d4ff]'
            }`}
                    ...
                    <div
                      ...
                      className={`rounded-md border transition-all ${
                        on
                          ...
                          : 'border-[#2a3a52] bg-[#1a2332] hover:border-[#00d4ff]/40'
                      }`}
                      style={{ '--c': p.color }}
                    >
                      <button
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <div
                            ...
                            style={{ background: p.ready ? p.color : '#4a5568' }}
                          />
                          <div className="text-left min-w-0">
                            <div className="text-[11px] font-bold truncate" style={{ color: p.color }}>
                              {p.label}
                            </div>
                        ...
                        {on ? (
                          <X size={12} className="text-[#ff1744] shrink-0" />
                        ) : (
                          <Plus size={12} className="text-[#6b82a6] shrink-0" />
                        )}
                      </button>
                      {on && (
                        <select
          ...
          <div className="flex-1 p-4 overflow-y-auto custom-scrollbar grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 auto-rows-min gap-3">
            {Object.entries(active).length === 0 && (
              <div className="col-span-full flex flex-col items-center justify-center text-center py-16 gap-3">
                ...
                <div className="text-xs tracking-widest uppercase text-[#6b82a6]">
                  Pick one or more platforms on the left
                </div>
              </div>
            )}
                ...
                >
                  <div className="bg-[#111827] border-b border-[#2a3a52] px-3 py-1.5 flex items-center justify-between">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-[11px] font-bold truncate" style={{ color: p?.color }}>
                        {p?.label || pid}
                      </span>
                      <span className="mono text-[9px] text-[#6b82a6] truncate">{model}</span>
                    </div>
                    {r?.latency_ms > 0 && (
                      <span className="mono text-[9px] text-[#ffb300] flex items-center gap-0.5 shrink-0">
                        <Zap size={8} />
                        {r.latency_ms}ms
                      </span>
                    )}
                  ...
                  <div className="p-3 text-[12px] leading-relaxed flex-1">
                    ...
                    {r?.state === 'loading' && (
                      <div className="flex gap-1 items-center text-[#6b82a6]">
                        ...
                        <span
                          className="w-1.5 h-1.5 bg-[var(--c,#00d4ff)] rounded-full animate-bounce"
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1 text-[9px] tracking-widest uppercase px-2 py-1 rounded-full border transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
        on
          ? 'border-[var(--c)] bg-[var(--c)]/15 text-[var(--c)]'
          : 'bg-[#1a2332] border-[#2a3a52] text-[#6b82a6] hover:border-[var(--c)]'
      }`}
      style={{ '--c': color }}
    >
      <Radio size={10} /> {label}
