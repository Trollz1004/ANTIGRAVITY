...
import { Shield, ShieldCheck, ShieldAlert, Globe, Lock, Radio, MessageSquare, Send, AlertCircle, Loader2, Check, X as XIcon } from "lucide-react";
  ...
  return (
    <div data-testid="security-mode" className="h-full overflow-y-auto bg-[#0a0f1a] text-[#e8f0ff]">
      <div className="max-w-5xl mx-auto px-8 py-10">
        <header className="flex items-start gap-4 flex-wrap mb-8">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] tracking-[0.3em] uppercase text-[#e040fb] font-bold">Boris&Dario · Zero-trust ledger guard</span>
            </div>
            <h1 className="text-3xl font-bold tracking-tight">Mission Security Posture</h1>
            <p className="text-sm text-[#6b82a6] mt-1">{posture?.tagline || "Zero-trust ledger guard · Business-only product operations protection layer"}</p>
          </div>
        ...
        {loading ? (
          <div className="flex items-center gap-2 text-[#6b82a6] text-xs"><Loader2 size={14} className="animate-spin" /> probing posture…</div>
        ) : posture && (
          <div className="space-y-6">
            <Section title="Caller fingerprint" Icon={Globe}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <Cell label="your ip" value={posture.client.ip} mono />
                <Cell label="behind cloudflare" value={posture.client.behind_cloudflare ? "yes" : "no"} tone={posture.client.behind_cloudflare ? "green" : "amber"} />
                <Cell label="https enforced" value={posture.client.https_enforced ? "yes" : "no"} tone={posture.client.https_enforced ? "green" : "red"} />
              </div>
            </Section>

            <Section title="Admin sign-in gate" Icon={Lock}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <Cell label="password set" value={posture.admin_gate.password_set ? "yes" : "no"} tone={posture.admin_gate.password_set ? "green" : "red"} />
                <Cell label="session secret" value={posture.admin_gate.session_secret_set ? "yes" : "no"} tone={posture.admin_gate.session_secret_set ? "green" : "red"} />
            ...
            <Section title="IP allowlist · zero-trust gate" Icon={ShieldCheck}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                <Cell label="active" value={posture.ip_allowlist.active ? "yes" : "no"} tone={posture.ip_allowlist.active ? "green" : "amber"} />
                <Cell label="entries" value={posture.ip_allowlist.entries.length || "—"} mono />
                <Cell label="your ip allowed" value={posture.ip_allowlist.current_ip_allowed ? "yes" : "blocked"} tone={posture.ip_allowlist.current_ip_allowed ? "green" : "red"} />
              </div>
              {posture.ip_allowlist.entries.length > 0 && (
                <div className="text-[10px] mono text-[#6b82a6] bg-[#0a0f1a] border border-[#2a3a52] rounded p-2 max-h-24 overflow-y-auto">
                  {posture.ip_allowlist.entries.map((e) => <div key={e}>{e}</div>)}
                </div>
              )}
        ...
        <footer className="mt-12 pt-6 border-t border-[#2a3a52] text-[10px] text-[#4a5568] tracking-widest uppercase">
          Boris&Dario protect the product reserve · Business-only product operations · no trust-me-bro
        </footer>
      </div>
    </div>
  );
...
function GradeBadge({ grade, score }) {
  ...
  return (
    <div data-testid="security-grade-badge" className="bg-[#111827] border-2 rounded-lg px-5 py-3 min-w-[140px] text-center"
      ...
      <div className="text-4xl font-black mono" style={{ color: tone }}>{grade}</div>
...
function Cell({ label, value, tone, mono }) {
  ...
  return (
    <div className="bg-[#0a0f1a] border border-[#2a3a52] rounded p-3">
      ...
      <div className={`${mono ? "mono" : ""} text-sm font-bold`} style={{ color }}>{value}</div>
    </div>
  );
}
...
function ChannelCard({ binding }) {
  ...
  return (
    <div data-testid={`channel-${binding.channel.toLowerCase()}`} className="bg-[#0a0f1a] border rounded p-3 flex flex-col gap-1.5"
      style={{ borderColor: wired ? "#00e67640" : "#2a3a5280" }}>
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <Icon size={12} style={{ color: wired ? "#00e676" : "#6b82a6" }} />
          <span className="text-xs font-bold" style={{ color: wired ? "#00e676" : "#6b82a6" }}>{binding.agent}</span>
        </div>
      ...
      <div className="text-[8px] mono text-[#4a5568]">{binding.destination_env}: {binding.destination_set ? "set" : "missing"}</div>
    </div>
  );
}
