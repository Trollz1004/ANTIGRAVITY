      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Counter
              label="Kids Covered (Est.)"
              value={String(stats.kids_estimate)}
              color="#00e676"
              big
              sub={`each = $${stats.kids_threshold_usd.toFixed(0)} medical-care unit`}
            />
            <Counter
              label="Active Buckets"
              value={String((stats.by_bucket || []).filter((b) => b.amount_usd > 0).length)}
              color="#ffb300"
            />
          </div>
        )}
          ...
          <div className="bg-[#1a2332] border border-[#2a3a52] rounded-md">
            ...
            <div className="p-3 grid grid-cols-2 sm:grid-cols-5 gap-2">
              {stats.by_bucket.map((b) => (
                <div
                  key={b.bucket}
                  data-testid={`ledger-bucket-${b.bucket}`}
                  className="aspect-[4/3] border border-[#2a3a52] bg-[#0a0f1a] rounded p-2.5 flex flex-col justify-between"
                >
                  <div className="flex items-center justify-between">
                    <span className="mono text-[9px] tracking-widest uppercase text-[#6b82a6]">
                      B{String(b.bucket).padStart(2, '0')}
                    </span>
                    {b.count > 0 && (
                      <span className="text-[8px] tracking-widest uppercase text-[#00e676]">{b.count}×</span>
                    )}
                  </div>
          ...
          <div className="bg-[#1a2332] border border-[#2a3a52] rounded-md p-3">
            ...
            <div className="flex gap-2 flex-wrap">
              {stats.by_source.map((s) => (
                <div
                  key={s.source}
                  className="bg-[#0a0f1a] border border-[#2a3a52] rounded px-2.5 py-1 flex items-center gap-2"
                >
        ...
        <div className="bg-[#1a2332] border border-[#2a3a52] rounded-md">
          ...
          {entries.length === 0 ? (
            <div className="p-6 text-center text-[10px] text-[#4a5568] italic">
              No contributions recorded yet. Wire your Square/Stripe/Cloudflare worker to{' '}
              <span className="mono text-[#fb923c]">/api/ledger/webhook/&lt;source&gt;</span> or click{' '}
              <span className="text-[#e040fb]">Record contribution</span> above.
            </div>
          ) : (
            <div className="divide-y divide-[#2a3a52]">
              {entries.map((e) => (
                <div
                  ...
                  className="px-3 py-2 flex items-center gap-3 hover:bg-[#0a0f1a]/50 transition-colors"
                >
                  <span
                    className="w-2 h-2 rounded-full shrink-0"
                    style={{ background: SOURCE_COLORS[e.source] || '#6b82a6' }}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] truncate">
                      <span className="mono text-[#00e676] font-bold">${formatUsd(e.amount_usd)}</span> →{' '}
                      <span className="text-[#e040fb]">{e.bucket_name}</span>{' '}
                      {e.note && <span className="text-[#6b82a6]">· {e.note}</span>}
                    </div>
                    <div className="mono text-[8px] tracking-widest uppercase text-[#4a5568]">
                      {e.source} · {new Date(e.at).toLocaleString()}
                    </div>
                  </div>
                  <button onClick={() => remove(e.id)} className="text-[#6b82a6] hover:text-[#ff1744] shrink-0">
                    <Trash2 size={10} />
                  </button>
                </div>
              ))}
...
function Counter({ label, value, color, big, sub }) {
  return (
    <div className="bg-[#1a2332] border border-[#2a3a52] rounded-md p-3">
      ...
      <div className="mono font-bold leading-tight" style={{ color, fontSize: big ? '2rem' : '1.5rem' }}>
        {value}
      </div>
...
function NewContributionModal({ onClose, onSaved }) {
  ...
  return (
    ...
    >
      <div
        className="bg-[#1a2332] border border-[#2a3a52] rounded-md max-w-md w-full overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="bg-[#111827] border-b border-[#2a3a52] px-4 py-2.5 text-xs font-bold tracking-wide">
          Record contribution
        </div>
            ...
            <Field label="Bucket">
              ...
              >
                {[
                  ...
                  [3, 'Hermes Ops'],
                  [4, 'Recycle Intake'],
                  [5, 'AI-Solutions Store'],
                  [6, 'Super Likes Match'],
                  [7, 'Content Sprint'],
                  [8, 'Support Automation'],
                  [9, 'Antigravity Reserve'],
...
const inputCls =
  'w-full bg-[#0a0f1a] border border-[#2a3a52] rounded px-2.5 py-1.5 text-sm text-[#e8f0ff] focus:border-[#e040fb] focus:ring-1 focus:ring-[#e040fb]/20 outline-none mono';
