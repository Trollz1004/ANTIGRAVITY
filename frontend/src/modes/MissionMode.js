...
export function MissionMode() {
  ...
  useEffect(() => {
    axios
      ...
      .then((r) => setDao(r.data))
      .catch(() => setDao(null));
    axios
      .get(`${API}/business/metrics`, { timeout: 4000 })
    ...
    <div data-testid="business-mode" className="h-full overflow-y-auto custom-scrollbar p-4 bg-[#0a0f1a]">
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col gap-4">
          ...
          <div
            className="bg-[#0a0f1a] border border-[#fb923c]/30 rounded-md p-3 relative overflow-hidden"
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <div className="mono text-[9px] text-[#6b82a6] leading-relaxed">
                Built this Mission Control. Now seated at the Roundtable — ask{' '}
                <span className="text-[#fb923c]">E1</span> what to ship next.
              </div>
          ...
          <div className="bg-gradient-to-br from-[#e040fb]/10 to-[#ffb300]/10 border border-[#e040fb]/30 rounded-md p-4 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="text-lg font-bold text-[#e8f0ff] leading-tight mb-2">YouAndINotAI</div>
              ...
              <div className="mt-3 pt-3 border-t border-[#e040fb]/20 text-[8px] tracking-widest uppercase text-[#6b82a6]">
                Runway · {business?.runway_days ?? '—'} days · primary ·{' '}
                {business?.primary_product ?? 'youandinotai.com'}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="p-3 grid grid-cols-2 lg:grid-cols-4 gap-3">
        {(dao?.tokens || []).map((t) => {
          ...
          return (
            <div
              key={t.symbol}
              data-testid={`dao-token-${t.symbol}`}
              className="bg-[#0a0f1a] border border-[#2a3a52] rounded p-3"
            >
              <div className="flex items-center justify-between mb-2">
                <span className="mono text-sm font-bold" style={{ color: t.color }}>
                  ${t.symbol}
                </span>
                <span className="text-[8px] tracking-widest uppercase text-[#6b82a6]">{pct.toFixed(1)}%</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 mono text-[9px] text-[#6b82a6]">
                <div>
                  <span className="text-[#4a5568]">circ</span> {t.circulating.toLocaleString()}
                </div>
                <div>
                  <span className="text-[#4a5568]">cap</span> {(t.cap / 1_000_000).toFixed(1)}M
                </div>
                <div>
                  <span className="text-[#4a5568]">rwd</span> {(t.activity_reserved / 1_000_000).toFixed(2)}M
                </div>
                <div>
                  <span className="text-[#4a5568]">fnd</span> {(t.founders / 1000).toFixed(0)}K
                </div>
              </div>
            </div>
          );
      ...
      {business && (
        <div className="px-3 py-2 border-t border-[#2a3a52] flex items-center justify-between text-[9px] tracking-widest uppercase">
          <span className="text-[#6b82a6]">
            tag · <span className="text-[#e040fb]">{business.tag}</span>
          </span>
          <span className="text-[#6b82a6]">
            runway · <span className="text-[#ffb300]">{business.runway_days} days</span>
          </span>
          <span className="text-[#6b82a6]">
            product · <span className="text-[#00d4ff]">{business.primary_product}</span>
          </span>
        </div>
      )}
    </div>
      ...
      <div className="p-3 grid grid-cols-2 sm:grid-cols-5 gap-1.5">
        {buckets.map((b) => (
          ...
          >
            ...
            <div className="text-[9px] tracking-wide leading-tight text-[#e8f0ff] font-bold">{b.name}</div>
          </div>
        ))}
        {buckets.length === 0 && (
          <div className="col-span-full text-[10px] text-[#4a5568] italic">endpoint unreachable — retry</div>
