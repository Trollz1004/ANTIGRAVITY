...
const PUBLIC_SURFACES = [
  ...
  },
  {
    ...
    description: "Separate storefront surface. Customer messaging should stay distinct from service and dashboard pages.",
  },
  {
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <section className={`p-8 rounded-[3rem] border ${isDarkMode ? "bg-slate-900/40 border-slate-800" : "bg-white border-slate-100 shadow-sm"}`}>
          ...
          <p className={`${isDarkMode ? "text-slate-300" : "text-slate-600"} max-w-3xl leading-relaxed`}>
            This dashboard is intentionally limited to verified public links and explicitly tracked numbers.
            Internal node topology, operational task logs, credential workflows, and unfinished financial proofs do not belong on a public surface.
          </p>
        </section>
            ...
            {PUBLIC_SURFACES.map((surface) => (
              <a
                ...
                className={`p-5 rounded-2xl border transition-all hover:-translate-y-1 ${
                  isDarkMode ? "bg-slate-950/50 border-slate-800 hover:border-blue-500" : "bg-slate-50 border-slate-200 hover:border-blue-400"
                }`}
              >
                ...
                <p className={`text-sm mt-3 ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>{surface.description}</p>
              </a>
            ))}
          </div>
        </section>
