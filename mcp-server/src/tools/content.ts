...
export function registerContentTools(server: McpServer) {
  ...
  server.tool(
    ...
    {
      platform: z
        .enum(['x', 'reddit', 'tiktok', 'linkedin', 'all'])
        .default('all')
        .describe("Filter by platform or 'all'"),
    },
    async ({ platform }) => {
      const filtered = platform === 'all' ? CONTENT_BANK : CONTENT_BANK.filter((p) => p.platform === platform);
  ...
  server.tool('content_get_tiktok_script', 'Get a TikTok video script from the content bank', {}, async () => {
